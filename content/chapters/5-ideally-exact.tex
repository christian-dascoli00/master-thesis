...
\end{tikzcd}
    ...
    Picking up where we left off, consider $m$ the mono arising from the (regular epi, mono) factorization of $!:0 \to 1$ in $\mathcal{A}$. Since $U$ preserves monos, then $U(m)$ is a mono. In particular, since the final object $1$ is a limit, it is preserved by $U$, then $U(m)$ is a mono with codomain $1$ in $\mathcal{X}$. Moreover, $\mathcal{X}$ is semiabelian, hence pointed, so $0=1$ in $\mathcal{X}$ and $U(m)$ is a mono with codomain $0$ in $\mathcal{X}$. This implies that $U(m)$ is an iso, in fact, suppose that $U(m)$ is a mono with codomain $0$ and let $U(C)$ be its domain. Denote by $!_{U(C)}$ the unique morphism from $U(C)$ to $0$, then $!_{U(C)}$ is the inverse of $U(m)$. In particular $!_{U(C)}U(m)=!_0=1_0$. Composing with $U(m)$ we obtain $U(m)!_{U(C)}U(m)=U(m)$, thus, observing that $U(m)!_{U(C)}$ and $1_{U(C)}$ are a morphisms between objects arising from $\mathcal{A}$, we also have that $U(m)!_{U(C)}=1_{U(C)}$. Thus $U(m)$ is an iso.
    \[
