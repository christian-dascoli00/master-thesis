...
\label{chap:ideally-ex}

\fontsize{12}{14.5}\selectfont

\section{Definition and characterization}

We are now ready to prove the theorem, which will lead to the definition of the ideally exact categories. The following is based on the work of Janelidze \cite{Janelidze2024}. The next theorem presents a slight variation of the original result.
...
\begin{mainbox}{}
    \begin{theo}\label{Ideally exact categories}
        ...
        \begin{enumerate}
            ...
            \item $\mathcal{A}$ is exact\footnotemark[1] and there exists $\mathcal{X}$ semiabelian category with a monadic functor $U: \mathcal{A} \to \mathcal{X}$.
            \item There exists $\mathcal{X}$ semiabelian category with a monadic functor $U: \mathcal{A} \to \mathcal{X}$ and $F$ left adjoint to $U$ such that the associated monad $T=UF$ preserves regular epimorphisms and kernel pairs.
            \[
...
\begin{tikzcd}
    ...
    \end{theo}
\end{mainbox}
\footnotetext[1]{Janelidze in condition \textit{2} requires that $\mathcal{A}$ has finite coproducts. As we will see in the proof, this requirement is not necessary, since one can still construct finite coproducts in \textit{2} $\implies$ \textit{1} in the same way as Janelidze does in \textit{3} $\implies$ \textit{2}. Nevertheless, this is equivalent to the author's claim: condition \textit{1} indeed implies that $\mathcal{A}$ has finite coproducts.}
...
\begin{subbox}{}
    \begin{proof}[\textit{Proof of \Cref{Ideally exact categories}}]\label{The proof}
        ...
        $(\mathcal{A} \downarrow 0)$ is pointed, in fact if $(A,\alpha)$ is an object in $(\mathcal{A} \downarrow 0)$, we have the following initial and terminal maps:
        \[
   % https://tikzcd.yichuanshen.de/#N4Igdg9gJgpgziAXAbVABwnAlgFyxMJZABgBpiBdUkANwEMAbAVxiRGJAF9T1Nd9CKAIzkqtRizYBBLjxAZseAkRFCx9Zq0TtZvRQKIBmUdQ2TtM7nv7KUxtaYladV+XyWDkAFhPjNbDk4xGCgAc3giUAAzACcIAFskERAcCCQAJkd-bQAdHMY0AAs6XRBYhKQyFLTEZLNnAEIAfUs5csTEY2qkAFYs8xA8guLS9srqVIz+5yEmjmoGOgAjGAYABXcDbRisUMKcUbiOvu7EHz8B2cC2o6QuybPptiGGIpKgziA
\begin{tikzcd}
0 \arrow[r, "!_A"] \arrow[rd, "1_0"'] & A \arrow[d, "\alpha"] &  & A \arrow[r, "\alpha"] \arrow[d, "\alpha"] & 0 \arrow[ld, "1_0"] \\
                                      & 0                     &  & 0                                         &                    
\end{tikzcd}
        \]
...
\end{tikzcd}
    ...
    We must now establish that the $(\mathcal{A} \downarrow 0)$ is protomodular. Since the construction of the change-of-base functor involves pullbacks, which are connected limits, then by \Cref{U creates con lim} the construction in $(\mathcal{A} \downarrow 0)$ is the same as in $\mathcal{A}$. Thus $(\mathcal{A} \downarrow 0)$ inherits protomodularity from $\mathcal{A}$, noting that the definition of iso in $\mathcal{A}$ and $(\mathcal{A} \downarrow 0)$ is essentially the same, except for the morphism condition. Hence, since $\mathcal{X} = (\mathcal{A} \downarrow 0)$ is exact, protomodular, pointed and has binary coproducts, we can conclude that it is semiabelian.
    ...
    We need to verify that $\mathcal{A}$ is exact. Since $U$ is monadic, we have that $\mathcal{A} \cong \mathcal{X}^T$. So it suffices to show that $\mathcal{X}^T$ is exact. We know that $\mathcal{X}$ is semiabelian, so it is exact. Then, by \Cref{algebras category is exact if the base category is exact}, $\mathcal{X}^T$ is exact.

    $\mathit{(2 \implies 1)}$ Suppose that $\mathcal{A}$ is exact and there exists a semiabelian category $\mathcal{X}$ and a monadic functor $U: \mathcal{A} \to \mathcal{X}$.
...
\begin{prop}\label{ideally exact has essentially nullary monad}
    ...
    There exist $\mathcal{X}$ semiabelian category with a monadic functor $U: \mathcal{A} \to \mathcal{X}$ and the corresponding monad is essentially nullary. Moreover, the functor $U$ is the pullback functor associated to $!: 0 \to 1$
\end{prop}
\end{mainbox}
...
\begin{subbox}{}
    \begin{proof}
        ...
        First, we aim to prove that $U^0$ reflects isos. Let $f:(A,\alpha)\to (A',\alpha')$ be a morphism in $\mathrm{Pt}_\mathcal{A}(0)$ such that $U^0(f)$ is iso. Observe that $U^0(f)=(!_{U(0)})^*(U(f))$. 
        \[
...
\begin{mainbox}{}
\begin{theo}\label{U is !^* up to iso iff unit is cartesian}
...
\begin{subbox}{}
    \begin{proof}
        We already proved that $(\mathcal{A} \downarrow 1) \cong \mathcal{A}$, by the unique morphism to the terminal object. Suppose that the unit $\eta$ associated to $U$ is cartesian, we aim to prove that $(\mathcal{A} \downarrow 0) \cong \mathcal{X}$ and that the diagram commutes up to natural isomorphisms.
...
\begin{rembox}{}
    \begin{rem}
        One can also prove directly that the downward-right directed square \eqref{U is !^* up to iso} commutes up to natural iso. So, we aim to prove that $U^0 !^*$ corresponds to the functor $U$. In order to do that we compute $U^0 !^*$ in an object $A$ in $\mathcal{A}$. Consider the object $(A,!^A)$, and its pullback
        \[
...
\begin{tikzcd}
0\times A \arrow[rr, "\mathrm{proj}_2"] \arrow[d, "\mathrm{proj}_1"] &  & A \arrow[d, "!^A"] \\
0 \arrow[rr, "!"]                                                     &  & 1                 
\end{tikzcd}
        \]
        Since $U$ is a right adjoint, also the following square is a pullback.
        \[
...
\begin{tikzcd}
U(0 \times A) \arrow[rr, "U(\mathrm{proj}_2)"] \arrow[d, "U(\mathrm{proj}_1)"] &  & U(A) \arrow[d, "U(!^A)"] \\
U(0) \arrow[rr, "U(!)"]                                                           &  & U(1)                    
\end{tikzcd}
\]
Then, apply $U^0$ to $(0\times A,\mathrm{proj}_1)$. By definition \eqref{induced ajunction on 0}, we need to compute the pullback of $U(\mathrm{proj}_1)$ along $!_{U(0)}$.

Now, the last diagram can be rewritten, up to iso, as
\[
% https://tikzcd.yichuanshen.de/#
\begin{tikzcd}
U(0) \times U(A) \arrow[rr, "\mathrm{proj}_2"] \arrow[d, "\mathrm{proj}_1"] &  & U(A) \arrow[d, "!^{U(A)}"] \\
U(0) \arrow[rr, "!^{U(0)}"]                                                     &  & 1                         
\end{tikzcd}
\]
so $U^0!^*(A) = U^0((0 \times A, \mathrm{proj}_1)) = \mathrm{Ker}(U(\mathrm{proj}_1)) \cong \mathrm{Ker}(\mathrm{proj}_1: U(0) \times U(A) \to U(0)) \cong U(A)$
\end{rem}
\end{rembox}

\Cref{U is !^* up to iso iff unit is cartesian} answers the question we posed at the beginning of this section. The following example shows that we already came across to such a scenario.
...
\begin{tikzcd}
(\mathsf{Ring} \downarrow \{0\}) \arrow[dd, "!^*", shift left] \arrow[rr, "\cong", no head]                       &  & \mathsf{Ring} \arrow[dd, "U", shift left]                                                    \\
                                                                                                              &  &                                                                                              \\
...
Notice that the example of rings given in the first chapter is consistent with the definition of ideally exact category, since the forgetful functor
\[
...
\end{tikzcd}
\]
is monadic, and $\mathsf{Ring}_*$ is semiabelian. Moreover $U$ is, up to iso, the pullback functor along $!:0 \to 1$, which makes applicable \Cref{quotients in ideally exact} in this case. 

In a semiabelian category, a \textbf{normal subobject} is precisely the kernel of a morphism. In this section we will investigate the notion of ideal. As we will see, this concept extends the familiar notion of normal subobject from semiabelian categories.

In an ideally exact category $\mathcal{A}$ we have the following structure, with semiabelian $\mathcal{X}$.
\[
\begin{tikzcd}
...
\begin{itemize}[itemsep=1.5ex]
    ...
    \item $n 1_X = n$
\end{itemize}
Since $[T(!_{X}),\eta_{X}]$ is a strong epi, there exists a unique morphism $\xi$ making the diagram commute. $\xi$ turns out to be an algebra for $X$. 
...
In the next results we consider the following lattices. Let $A$ be an object in a category $\mathcal{A}$.
\begin{itemize}[itemsep=1.5ex]
...
\]
    ...
    that associates to each kernel of some morphism $f:X \to Y$ the regular epi $p$ underlying the corresponding (regular epi, mono) factorization of $f$.
    \[
...
\begin{theo}\label{quotients in ideally exact}
    Let $\mathcal{A}$ be an ideally exact category and let $U:\mathcal{A} \to \mathcal{X}$ be, up to equivalence, the underlying pullback functor associated to $! : 0 \to 1$. Then for each object $A$ in $\mathcal{A}$ the following lattices are isomorphic.
...
\begin{subbox}{}
    \begin{proof}
        ...
        because $\mathcal{A}$ is protomodular, hence Mal'tsev. By \Cref{ideally exact has essentially nullary monad} the monad of the adjunction associated to $U$ is essentially nullary. Then, by \Cref{T-algebra in the reflexive relation}, we have that
        \[\mathrm{RRel}(A)\cong \mathrm{RRel}(U(A)) \]
        where we also used the monadicity of $U$ to conclude that the category of algebras is equivalent to $\mathcal{A}$.
...
Given an ideally exact category $\mathcal{A}$, the elements of $\mathrm{NSub}(U(A))$ are called $U$\textbf{-ideals} of $\mathcal{A}$. If $U$ is, up to iso, the pullback functor along $! : 0 \to 1$, then these are simply called \textbf{ideals}.

The previous theorem gives the correspondence between $U$-ideals and regular epis, which play the role of quotients.
...
\begin{rembox}{}
    \begin{rem}
        In $\mathsf{Ring}$, given an object $A$ in $\mathcal{A}$, the lattice $\mathrm{NSub}(U(A))$ consists of ideals. As shown in the first chapter, the forgetful functor $U:\mathsf{Ring} \to \mathsf{Ring_*}$ is, up to equivalence, the pullback functor associated to $! : 0 \to 1$. We have the following corresponding morphisms, composed by an ideal $I$, i.e. a kernel, and its corresponding quotient map, i.e. a regular epi, $\pi$.
        \[
...
With this final theorem we proved that in an ideally exact category quotients and normal subobjects correspond to each others. This is exactly what happens in $\mathsf{Ring}$. In fact the ideals in $\mathsf{Ring}$, which are the normal subobjects in $\mathsf{Ring_*}$ through the forgetful functor, correspond to quotients, which are the regular epis. Observe that in $\mathsf{Grp}$ the result holds directly from \Cref{quotients in semiabelian}, since $\mathsf{Grp}$ is a semiabelian category, and normal subgroups are objects of $\mathsf{Grp}$. Thus in this case we do not need to consider an underlying functor, or, equivalently, we can just consider the identity functor.