...
As illustrated in \Cref{Ideally exact categories}$\mathit{:(1\implies 3)}$, in the definition of an ideally exact category, the functor involved can be taken to be the pullback functor induced by the unique morphism $!:0 \to 1$. On the other hand, one can also consider a monadic functor $U$ satisfying certain properties, as in \Cref{Ideally exact categories}\textit{:(2),(3)}. A natural question arises: is this monadic functor $U$, up to equivalence of categories, the same as the pullback functor associated to the unique morphism $!:0 \to 1$? In this section, we aim to address this question.
...
\begin{rembox}{}
    \begin{rem}
        One can also prove directly that downward-right directed square \eqref{U is !^* up to iso} commutes up to natural iso. So, we aim to prove that $U^0 !^*$ correspond to the functor $U$. In order to do that we compute $U^0 !^*$ in an object $A$ in $\mathcal{A}$. Consider the object $(A,!^A)$, and its pullback
        \[
