...
We already saw in \Cref{Ideally exact categories} that an we can consider as functor of the definition the one induced by the morphism $!:0 \to 1$, to form the pullback functor. One can ask whether or not this functor is, up to equivalence of categories, the pullback functor. We aim to answer this question in this section.
...
\end{tikzcd}
\]
Since $F$ is a left adjoint, we can assume $F(0_\mathcal{X})=0_\mathcal{A}$. In the following the category of the $0$ will be implicit.
...
\end{tikzcd}
\end{equation}
One can prove that this is an adjunction. Given $\eta$ and $\epsilon$ the unit and counit of $F \dashv U$, then the unit of $F^0 \dashv U^0$ in an object $X$ in $\mathcal{X}$ is
...
\begin{subbox}{}
    \begin{proof}
        ...
        First, we aim to prove that $U^0$ reflects iso. Let $f:(A,\alpha)\to (A',\alpha')$ be a morphism in $\mathrm{Pt}_\mathcal{A}(0)$. Observe that $U^0(f)=(!_{U(0)})^*(U(f))$. 
        \[
...
\begin{mainbox}{}
\begin{theo}
Let $\mathcal{A}$ be an ideally exact category, and let $U:\mathcal{A} \to \mathcal{X}$ be the monadic functor associated to the definition. The functor $U$ is, up to category equivalence, the pullback functor $!^*$ if and only if the unit associated to $U$ is cartesian.
\[
...
\begin{subbox}{}
    \begin{proof}
        ...
        We need to prove that such equivalence of categories makes the diagram commute.
        \[
...
\end{tikzcd}
        \]
        The upward-left directed square commutes up to natural iso by definition of $F^0$. 
        
        It remains to prove that the downward-right directed square also commutes up to natural iso. In particular we aim to prove that $U^0 !^*$ correspond to the functor $U$. In order to do that we compute $U^0 !^*$ in an object $A$ in $\mathcal{A}$. Consider the object $(A,!^A)$, and its pullback
        \[
...
Conversely, suppose that $U$ is, up to natural iso, the pullback functor of $!$, then, by \Cref{eta cartesian}, $\eta$ is cartesian.
    \end{proof}
\end{subbox}

\begin{rembox}{}
    \begin{ex}
In the first chapter we saw the same theorem in the particular case of $\mathcal{A}=\mathbf{Ring}$ and $\mathcal{X}=\mathbf{Ring_*}$, where $1=\{0\}$ and $0=\mathbb{Z}$. 
\[
