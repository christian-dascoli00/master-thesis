...
\end{tikzcd}
\]
we can consider the adjunction induced on the initial objects $0$, defined as follows. Let $X$ be an object in $\mathcal{X}$ and let $(A,\alpha)$ be an object in $(\mathcal{A}\downarrow 0)$. Since $\mathcal{X}$ is pointed, then $\mathcal{X} \cong (\mathcal{X}\downarrow 0)$. Denote $!^X:X \to 0$ the unique morphism in the zero object.
\begin{align*}
F^0(X) &= (F(X), F(!^X)) \\[1.5ex]
...
\end{tikzcd}
\]
Since $F$ is a left adjoint, we can assume $F(0)=0$.
\[
