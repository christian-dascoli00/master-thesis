...
\begin{tikzcd}
\mathbf{Ring} \arrow[r, "U"] & \mathbf{Ring_*}
\end{tikzcd}
\]
...
\end{tikzcd}
\]
Observe that ${\langle 1_X,1_X\rangle}$ is a $T$-algebra morphism, in fact given the product structure \eqref{product lift} of $X\times X$ on the category $\mathcal{X}^T$
\[
...
\begin{itemize}[itemsep=1.5ex]
    \item $\mathrm{Quot}(A)$: regular epi, up to iso, with domain $A$;
...
The following theorem shows the correspondence between the lattices of quotients and normal subobjects in a semiabelian category. This is a generalization of the classical result in $\mathbf{Grp}$.
...
With this final theorem we proved that in an ideally exact category quotients and normal subobjects correspond to each others. This is exactly what happen in $\mathbf{Ring}$ as mentioned in the first chapter. In fact in $\mathbf{Ring}$ the ideals, which are the normal subobjects in $\mathbf{Ring_*}$ through the forgetful functor, correspond to quotients, which are the regular epis. Observe that in $\mathbf{Grp}$ the result holds directly from \Cref{quotients in semiabelian}, since $\mathbf{Grp}$ is a semiabelian category. Thus in this case we do not need to consider an underlying functor, or, equivalently, we can just consider the identity functor.