...
\begin{mainbox}{}
\begin{defi}
    A \textbf{connected limit} is a limit over a connected category. A category is connected if it is inhabited (i.e. it is non-empty) and any two objects can be connected by a zigzag of morphisms
    \[
    \begin{tikzcd}
...
\begin{mainbox}{}
    \begin{prop}\label{U creates con lim}
       Let $E$ be an object in $\mathcal{A}$. Then the forgetful functor $U : (\mathcal{A} \downarrow E) \to \mathcal{A}$ creates 
       \begin{enumerate}
       \item all the connected limits that $\mathcal{A}$ has;
       \item all the colimits that $\mathcal{A}$ has.
    \end{enumerate}
\end{prop}
\end{mainbox}
...
\begin{subbox}{}
    \begin{proof}
        \textit{1}. The connected limits in the category $(\mathcal{A} \downarrow E)$ can be computed in $\mathcal{A}$. Consider a connected limit cone $(L,(q_i)_{i \in \mathcal{I}})$. Denote each object in $(\mathcal{A} \downarrow E)$ associated to $i \in \mathcal{I}$ as $(F(i),\varphi_i)$. Since the limit is connected, it is the limit over a connected, hence inhabited, category. Thus we can pick $j\in \mathcal{I}$. The limit in $(\mathcal{A} \downarrow E)$ is $((L,\varphi_jq_j),(q_i)_{i \in \mathcal{I}})$. The connection in the zigzag guarantees that all the morphisms $q_1, \ldots, q_n$ involved in the limit in $\mathcal{A}$ are also morphisms in $(\mathcal{A} \downarrow E)$, i.e. $\varphi_iq_i = \varphi_jq_j \ \forall i $.
    ...
    \[
        \begin{tikzcd}
            L \arrow[dd, "q_1" description, bend right] 
              ...
              \arrow[dddd, "\varphi_jq_j"', dashed, bend right=65] 
              & & & & \\
            & & & & \\
            F(1) 
    ...
    \]
        ...
        \textit{2}. The colimits in the category $(\mathcal{A} \downarrow E)$ can be computed in $\mathcal{A}$. Consider a colimit cocone $(L,(q_i)_{i \in \mathcal{I}})$. Denote each object in $(\mathcal{A} \downarrow E)$ associated to $i \in \mathcal{I}$ as $(F(i),\varphi_i)$. Observe that $(E,(\varphi_i)_{i \in \mathcal{I}})$ is a cocone over the diagram $F$. By the universal property, there exists a unique $\lambda: L \to E$ such that $\lambda q_i = \varphi_i$ for all $i \in \mathcal{I}$. The colimit in $(\mathcal{A} \downarrow E)$ is $((L,\lambda),(q_i)_{i \in \mathcal{I}})$.
        \[
        % https://tikzcd.yichuanshen.de/#N4Igdg9gJgpgziAXAbVABwnAlgFyxMJZABgBpiBdUkANwEMAbAVxiRADEAKLAShAF9S6TLnyEUARnJVajFmwAyAoSAzY8BImQkz6zVohABRATJhQA5vCKgAZgCcIAWyRkQOCEimz9bAI4A+ljKdo4uiG4eSABM1HryhgA6ifT2aAAWWEEg1Ax0AEYwDAAKIhriIPZYFuk4ISAOzl7UUYixPgkgyXlO+VB0pvxAA
\begin{tikzcd}
F(i) \arrow[r, "q_i"] \arrow[d, "\varphi_i"'] & L \arrow[ld, "\lambda"] \\
E                                             &                        
\end{tikzcd}
\]
    \end{proof}
...
Note that this result does not hold for all the limits. A counterexample is given in the following example.
\begin{rembox}{}  
    \begin{ex}
...
\begin{tikzcd}
    & D\times_ED' \arrow[ld] \arrow[r] & D' \arrow[ld, "\delta'"'] \arrow[ldd, "\delta'"] \\
...
\section{Pullback functor’s adjunction}\label{pullback functor}

Given a category $\mathcal{A}$ with pullbacks and a morphism $p: E \to B$ in $\mathcal{A}$, one can consider the pullback and shrink functors
\[
\begin{tikzcd}
...
\begin{tikzcd}
    A \arrow[d, "\alpha"] & {} \arrow[r, "p^*", maps to, shift right=7] & {} & E\times_BA \arrow[rr, "\mathrm{proj}_2"] \arrow[d, "\mathrm{proj}_1=p^*(\alpha)"'] &  & A \arrow[d, "\alpha"] \\
    B                     &                                             &    & E \arrow[rr, "p"]                                   &  & B                    
    \end{tikzcd}
\]
\[
...
\begin{tikzcd}
    D \arrow[d, "\delta"] & {} \arrow[r, "p_!", maps to, shift right=5] & {} & D \arrow[rd, "p\delta=p_!(\delta)"] \arrow[d, "\delta"'] &   \\
    E                     &                                             &    & E \arrow[r, "p"]                             & B
    \end{tikzcd}
\]
\fontsize{11}{13}\selectfont
...
\begin{subbox}{}
\begin{proof}
    By \Cref{U creates con lim} connected limits are computed in $\mathcal{A}$ and $p_!$ doesn't affect objects and morphisms in $\mathcal{A}$.
\end{proof}
\end{subbox}

\begin{mainbox}{}
\begin{prop}\label{adjunction of p}
...
\begin{minipage}{0.45\textwidth}
    ...
    \]
    \[
        \eta_{(D,\delta)} = \langle \delta,1_D \rangle \]
         \[ \langle \delta,1_D \rangle : (D,\delta) \to (E\times_BD, \mathrm{proj}_{1})
    \]
    \end{minipage}
...
\begin{minipage}{0.45\textwidth}
    ...
    \]
    \[
        \epsilon_{(A,\alpha)} = \mathrm{proj}_2 \] 
        \[ \mathrm{proj}_2 : (E\times_BA, \ p (\mathrm{proj}_1)) \to (A,\alpha)
    \]
    \end{minipage}
    \begin{minipage}{0.5\textwidth}
    \[
...
\begin{subbox}{}
\begin{proof}
We want to prove this is an adjunction. We start proving that the unit is a natural transformation. Let $g : (D,\delta)  \to (D',\delta')$ be a morphism in $(\mathcal{A} \downarrow E)$, we want to prove that the following diagram commutes:
\[
...
Now we want to prove that the counit is a natural transformation. Consider $f : (A,\alpha) \to (A',\alpha')$ a morphism in $(\mathcal{A} \downarrow B)$, we want to prove that the following diagram commutes:
\[
...
Now we can check that the unit and counit fulfil the first triangle identity. Let $(A,\alpha)$ be an object in $(\mathcal{A} \downarrow B)$.
...
Considering first how $p^*$ acts on the morphism $\mathrm{proj}_{2,E\times_BA}$
\[
...
E\times_BA \arrow[ru, "{\ \ \ \ \ \ \langle \mathrm{proj}_{1,E\times_BA},1_{E\times_BA}\rangle}" description] \arrow[rrrd, "{\mathrm{proj}_{1,E\times_BA}}"', bend right] \arrow[rru, "1_{E\times_BA}", bend left=67, crossing over] &                                                                                                                                                        &                                                                                                & B                &                        &                                                                        \\
    &                                                                                                                                                        &                                                                                                & E \arrow[u, "p"] &                        &                                                                       
\end{tikzcd}
\]

We want to check that the composition of the projections of $E\times_BA$ with $\circledast$ is the same as composing those projections with $1_{(E\times_BA,\mathrm{proj}_1)}$. Then, since the projections are jointly monic, we have the result. Following the first composition
\begin{align*}
    (&\mathrm{proj}_{2,E \times_BA})(\tilde{\mathrm{proj}}_{2,E\times_BA}) \langle \mathrm{proj}_{1,E\times_BA}, 1_{E\times_BA} \rangle = \\[1.5ex]
    & = (\mathrm{proj}_{2,E \times_BA}) (\mathrm{proj}_{2,E\times_B(E\times_BA)}) \langle \mathrm{proj}_{1,E\times_BA}, 1_{E\times_BA} \rangle \\[1.5ex]
    &= (\mathrm{proj}_{2,E \times_BA}) (1_{E\times_BA}) =(\mathrm{proj}_{2,E \times_BA}) (1_{(E\times_BA,\mathrm{proj}_1)})
\end{align*}
and the second composition
\begin{align*}
    (&\mathrm{proj}_{1,E \times_BA})(\tilde{\mathrm{proj}}_{2,E\times_BA}) \langle \mathrm{proj}_{1,E\times_BA}, 1_{E\times_BA} \rangle = \\[1.5ex]
    &= (\mathrm{proj}_{1,E\times_B(E\times_BA)}) \langle \mathrm{proj}_{1,E\times_BA}, 1_{E\times_BA} \rangle \\[1.5ex]
    &= \mathrm{proj}_{1,E \times_BA} \\[1.5ex]
...
Finally we can check the second triangle identity. Let $(D,\delta)$ be an object in $(\mathcal{A} \downarrow E)$, we have:
\[
...
Let $p:E\to B$ be a morphism in a category $\mathcal{A}$. The diagram constructed by $p^*$ acting on a morphism $f : (A,\alpha) \to (A',\alpha')$ is a pullback.
\[
...
\begin{subbox}{}
\begin{proof}
   Since the right and the total diagrams below are pullback, then the left diagram is a pullback too.
\[
...
\begin{mainbox}{}
\begin{propr}\label{eta cartesian}
Let $p:E\to B$ be a morphism in $\mathcal{A}$ with pullbacks. Then the unit $\eta$ and the counit $\epsilon$ of the adjuction associated to $p^*$ are cartesian.
\end{propr}
\end{mainbox}
\begin{subbox}{}
\begin{proof}
...
\begin{mainbox}{}
\begin{propr}
Let $p : E \to B$ be a morphism in $\mathcal{A}$ with pullbacks. Then the associated monad $(p^*p_!, \eta, p^*\epsilon p_!)$ is cartesian.
\end{propr}
\end{mainbox}
\begin{subbox}{}
\begin{proof}
We need to prove that the category $(\mathcal{A} \downarrow E)$ has pullbacks, the monad preserves pullbacks, and both the multiplication $p^* \epsilon p_!$ and the unit $\eta$ are cartesian.
...
In this section we look for conditions such that the monad associated to a morphism $p:E\to B$ is essentially nullary. This is a key ingredient of \Cref{induced algebra between morphisms}. This section is based on \cite{Janelidze2024}.

\begin{mainbox}{}
\begin{defi}
A monad $(T,\eta,\mu)$ on a category $\mathcal{X}$ with finite coproducts is said to be \textbf{nullary} if, for each object $X$ in $\mathcal{X}$, the morphism $[T(!_X),\eta_X]$ is an iso, where $!_X$ is the unique morphism from the initial object of $\mathcal{X}$ to $X$. A monad on a category $\mathcal{X}$ with finite coproducts is said to be \textbf{essentially nullary} if, for each object $X$ in $\mathcal{X}$, the morphism $[T(!_X),\eta_X]$ is a strong epi.
\end{defi}
\end{mainbox}

The following lemma, from \cite{BorceuxBourn2004}, will be useful for our purposes. 
\begin{mainbox}{}
\begin{lemma}\label{protomod lemma}
    Let $\mathcal{X}$ be a protomodular category. Then for each commutative diagram of the form
    \[
...
\begin{subbox}{}
\begin{proof}
Assume $\mathcal{X}$ is protomodular. We want to prove that if $u$ and $s$ factorize through a mono $m$ as $u = mf$ and $s = ms'$, then $m$ is an iso, which means $(u,s)$ is an extremally epimorphic pair. By a standard result, proved in \Cref{Eqivalence strong epi pair with iso}, this implies that $(u,s)$ is a strongly epimorphic pair.
...
\begin{tikzcd}
    V' \arrow[rrdd, "q'", bend right, shift left] \arrow[r, "w"'] \arrow[rrrr, "v^*(m)" description, dashed, bend left] & X' \arrow[rd, "p'", shift left] \arrow[rr, "m", tail] &                                                                                                        & X \arrow[ld, "p"', shift right] & V \arrow[lldd, "q"', bend left, shift right] \arrow[l, "u"] \arrow[lll, "f"', bend right=67, crossing over] \\
...
\begin{tikzcd}
    V \arrow[rrrd, "1_V", bend left] \arrow[rdd, "f", bend right] \arrow[rd, "{\langle f,1_V \rangle}", dashed] &                                       &  &                  \\
                                                                                                                & V' \arrow[d, "w"] \arrow[rr, "v^*(m)"] &  & V \arrow[d, "u"] \\
                                                                                                                & X' \arrow[rr, "m", tail]              &  & X               
    \end{tikzcd}
\]
Since the square is a pullback, it's obvious that $v^*(m) \langle f, 1_{V} \rangle  = 1_V$. It remains to prove that $\langle f, 1_{V} \rangle v^*(m) = 1_{V'}$. 
...
\begin{itemize}
    \item $v^*(m) (\langle f, 1_{V} \rangle v^*(m) ) = ( v^*(m) \langle f, 1_{V} \rangle ) v^*(m)  = 1_V v^*(m) = v^*(m) 1_{V'}$
    \item $w (\langle f, 1_{V} \rangle v^*(m)) = (w \langle f, 1_{V} \rangle ) v^*(m) = f v^*(m) = w = w 1_{V'} $
\end{itemize}
Since the pair $(v^*(m),w)$ is jointly monic we have that $\langle f, 1_{V} \rangle v^*(m) = 1_{V'}$. Considering this composition and the one above, it follows that $v^*(m)$ is an iso.

By the protomodularity of $\mathcal{X}$, since $v^*(m)$ is an iso, we have that $m$ is an iso, as claimed.
\end{proof}
\end{subbox}

We now aim to prove a sufficient condition for the monad associated to a morphism $p:E\to B$ to be essentially nullary. We could start with the following proposition.
...
\begin{subbox}{}
\begin{proof}
    For each object $X$ in $\mathcal{X}$ we can consider the following commutative diagram:
    \[
...
In our case $(A \downarrow E)$ is pointed if and only if $E$ is a an initial object, in fact in this case the following initial and terminal objects coincide.
\[
% https://tikzcd.yichuanshen.de/#
\begin{tikzcd}
E \arrow[r, "!_A"] \arrow[rd, "!_E"'] & A \arrow[d, "\alpha"] &  & A \arrow[r, "\alpha"] \arrow[d, "\alpha"] & E \arrow[ld, "1_E"] \\
                                      & E                     &  & E                                         &                    
\end{tikzcd}
\]

While this would work in that specific case, we would like to drop such condition. The following result fits our needs.

\begin{mainbox}{}
\begin{theo}\label{p^* is essentially nullary}
Let $p:E\to B$ be a morphism in $\mathcal{A}$ protomodular with finite coproducts. Then the monad associated to $p$ is essentially nullary.
\end{theo}
\end{mainbox}

\begin{subbox}{}
\begin{proof}
Let $(D,\delta)$ be an object in $(\mathcal{A} \downarrow E)$. To prove that the monad is essentially nullary we need to study the morphism of the initial object. So first note that the morphism for each object $(D,\delta)$ in $(\mathcal{A} \downarrow E)$ the morphism of the initial object $!_D$ is the same as the morphism of the initial object in $\mathcal{A}$, in fact the following diagram commutes.
\[
% https://tikzcd.yichuanshen.de/#N4Igdg9gJgpgziAXAbVABwnAlgFyxMJZABgBpiBdUkANwEMAbAVxiRGJAF9T1Nd9CKAIykhVWoxZsAolx4gM2PASIjK1es1aIQAES7iYUAObwioAGYAnCAFskAJmo4ISERK1sAOl9gMcdHKWNvaIZCAujhqS2iAAhAD6+tzBdkjhkYjumlI6ibLUDHQARjAMAAp8yoIgVljGABY4BpxAA
\begin{tikzcd}
0 \arrow[r, "!_D"] \arrow[rd, "!_E"'] & D \arrow[d, "\delta"] \\
                                      & E                    
\end{tikzcd}
\]
...
\[
% https://tikzcd.yichuanshen.de/#
\begin{tikzcd}
E\times_B0 \arrow[rr, "p^*p_!(!_D)"] \arrow[d, "\mathrm{proj}_2"', shift right] &  & E\times_BD \arrow[d, "\mathrm{proj}_2'"', shift right]     \\
0 \arrow[rr, "!_D"] \arrow[u, "{\langle !_E,1_0 \rangle}"', shift right]        &  & D \arrow[u, "{\langle \delta, 1_D \rangle}"', shift right]
\end{tikzcd}
\]
The downward directed square is commutative by the definition of $p^*$, and it is a pullback by \Cref{p^*(f) is a pullback}. The upward directed square commutes by naturality of $\eta$, in fact $\eta_{(0,!_E)}=\langle !_E, 1_0 \rangle$ and $\eta_{(D,\delta)}=\langle \delta, 1_D \rangle$. So, by \Cref{protomod lemma}, the pair $(p^*(!_D),\langle !_E,1_0 \rangle)$ is strongly epimorphic. By \Cref{Equivalence strong epi with coprod} this means that $[T(!_D),\eta_{(D,\delta)}]=[p^*p_!(!_D),\langle \delta,1_D \rangle]$ is a strong epi. Thus the monad is essentially nullary.
\end{proof}
\end{subbox}