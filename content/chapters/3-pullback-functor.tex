...
Note that this result does not hold for all the limits. This is extensively shown in the following example.
\begin{rembox}{}  
    \begin{ex}
...
\begin{subbox}{}
\begin{proof}
    By \Cref{U creates con lim} connected limits are computed in $\mathcal{A}$ and $p_!$ doesn't affect objects and morphism in $\mathcal{A}$.
\end{proof}
\end{subbox}
...
In this section we look for conditions such that the monad associated to a morphism $p:E\to B$ is essentially nullary. This will be useful in the main theorem of the ideally exact categories. This section is based on the work of \cite{Janelidze2024}.
...
\begin{tikzcd}
    W \arrow[rrdd, "q'", bend right, shift left] \arrow[r, "w"'] \arrow[rrrr, "v^*(m)" description, dashed, bend left] & X' \arrow[rd, "p'", shift left] \arrow[rr, "m", tail] &                                                                                                        & X \arrow[ld, "p"', shift right] & V \arrow[lldd, "q"', bend left, shift right] \arrow[l, "u"] \arrow[lll, "f"', bend right=67, crossing over] \\
...
\end{itemize}
Since the pair $(v^*(m),w)$ is jointly monic we have that $\langle f, 1_{V} \rangle v^*(m) = 1_W$. Considering this composition and the one above, it follows that $v^*(m)$ is an iso.
...
In our case $(A \downarrow E)$ is pointed if and only if $E$ is a an initial objects, in fact in this case the following initial and terminal objects coincide.
\[
...
\begin{subbox}{}
\begin{proof}
Let $(D,\delta)$ be an object in $(\mathcal{A} \downarrow E)$. To prove that the monad is essentially nullary we need to study the morphism of the initial object. So first note the morphism for each object $(D,\delta)$ in $(\mathcal{A} \downarrow E)$ the morphism of the initial object $!_D$ is the same as the morphism of the initial object in $\mathcal{A}$, in fact the following diagram commutes.
\[
...
\end{tikzcd}
\]
The downward directed square is commutative by the definition of $p^*$, and it is a pullback by \Cref{p^*(f) is a pullback}. The upward directed square commute by naturality of $\eta$, in fact $\eta_{(0,!_E)}=\langle !_E, 1_0 \rangle$ and $\eta_{(D,\delta)}=\langle \delta, 1_D \rangle$. So, by \Cref{protomod lemma}, the pair $(p^*(!_D),\langle !_E,1_0 \rangle)$ is strongly epimorphic. By \Cref{Equivalence strong epi with coprod} this means that $[T(!_D),\eta_{(D,\delta)}]=[p^*p_!(!_D),\langle \delta,1_D \rangle]$ is a strong epi. Thus the monad is essentially nullary.
\end{proof}
\end{subbox}