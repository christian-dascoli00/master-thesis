...
\begin{subbox}{}
    \begin{proof}
        \textit{1}. The connected limits in the category $(\mathcal{A} \downarrow E)$ can be computed in $\mathcal{A}$. Consider a connected limit cone $(L,(q_i)_{i \in \mathcal{I}})$. Denote each object in $(\mathcal{A} \downarrow E)$ associated to $i \in \mathcal{I}$ as $(F(i),\varphi_i)$. Since the limit is connected, it is the limit over a connected, hence inhabited, category. Thus we can pick $j\in \mathcal{I}$. The limit in $(\mathcal{A} \downarrow E)$ is $((L,\varphi_jq_j),(q_i)_{i \in \mathcal{I}})$. The connection in the zigzag guarantees that all the morphisms $q_1, \ldots, q_n$ involved in the limit in $\mathcal{A}$ are also morphisms in $(\mathcal{A} \downarrow E)$, ie $\varphi_iq_i = \varphi_jq_j \ \forall i $.
...
Given a category $\mathcal{A}$ with pullbacks and a morphism $p: E \to B$ in $\mathcal{A}$, one can consider the functors pullback and shrink
\[
\begin{tikzcd}
...
We want to check that composition of the projections of $E\times_BA$ with $\circledast$ is the same as composing those projections with $1_{(E\times_BA,\mathrm{proj}_1)}$. Then, since the projections are jointly monic, we have the result. Following the first composition
\begin{align*}
