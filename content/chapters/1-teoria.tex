...
Now we can claim some properties of the adjunction.

\begin{propr}
The unit $\eta$ and the counit $\epsilon$ are cartesian.
\begin{proof}

In order to prove that $\eta$ is cartesian, ie the naturality diagrams are pullback. Let $g : (D,\delta) \to (D',\delta')$ be a morphism in $(\mathcal{A} \downarrow E)$. Considering the commutative diagram below, since both the bottom and the total diagrams are pullback, it follows that the top diagram is a pullback.
\[
...
Considering the monad associated to the adjunction, we have have the following property.
