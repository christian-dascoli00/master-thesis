...
In this section we will define the pullback functor and its left adjoint, the shrink functor. Given a category $\mathcal{A}$ with pullbacks and a morphism $p: E \to A$ in $\mathcal{A}$, one can consider adjunction
\[
\begin{tikzcd}
...
Consider the monad associated to the adjunction we just studied, we want to study some properties that will be useful. In order to do that we need first to define some notions.
...
\[
x_0 = y_0 \leftrightarrow y_1 \leftrightarrow \cdots \leftrightarrow y_n = x_1
\]
where each symbol $\leftrightarrow$ is a morphism $x_i \to x_{i+1}$ or  $x_{i+1} \to x_i$.
\end{defi}
\begin{ex}  
    Some examples
    \begin{itemize}
...
\begin{tikzcd}
    ...
    \end{tikzcd}\]
        \item The product is not a connected limit.
        \[
...
\begin{rem}
The connected limits in the category $(\mathcal{A} \downarrow E)$ can be computed in $\mathcal{A}$. In fact we can provide to the objects in $\mathcal{A}$ a morphism to fomr an object in $(\mathcal{A} \downarrow E)$. In the diagram below we can choose for example $(\varphi_1)(q_1)$, that said we could use whatever couple of morphism $(\varphi_i)(q_i)$. The connection guarantee that all the morphisms $q_1 \ldots q_n$ involved in the limit are also morphisms in $(\mathcal{A} \downarrow E)$, ie each diagram composed by a pair of base moprhism $\varphi_1 \ldots \varphi_n$ and one of $q_1 \ldots q_n$ commute.
...
\[
    \begin{tikzcd}
        L \arrow[dd, "q_1" description, bend right] 
          ...
          \arrow[dddd, "(\varphi_1)(q_1)"', dashed, bend right=60] 
          & & & & \\
        & & & & \\
        F(1) 
          \arrow[leftrightarrow, rd, "F(i)" description] 
          \arrow[dd, "\varphi_1" description, bend right=49] 
          & 
          & F(2) 
            \arrow[leftrightarrow, ld, "F(j)" description] 
            \arrow[lldd, "\varphi_3" description, bend left=49] 
            \arrow[leftrightarrow, rd, "F(k)" description] 
          & 
          & F(n) 
            \arrow[lllldd, "\varphi_n" description, bend left=49] \\
        & F(2) 
          \arrow[ld, "\varphi_2" description, bend right] 
          & 
          & \cdots 
            \arrow[leftrightarrow, ru, "F(l)" description] 
          & \\
        E & & & & 
        \end{tikzcd}
\]
...
The category $(\mathcal{A} \downarrow E)$ has pullback, since it pullbacks are computed in $\mathcal{A}$, and $\mathcal{A}$ has pullbacks. The monad $p^*p_!$ preserves pullbacks, in fact $p^*$ is a right adjoint so it preserves limits, therefore preserves pullbacks, and $p_!$ preserves pullbacks because it only affects the morphism with the base $E$. We already proved that the unit $\eta$ is cartesian, so it remains to prove that the multiplication $p^* \epsilon p_!$ is cartesian. Since $\epsilon$ is cartesian and $p^*$ preserves pullbacks as shown above, we can conclude that the multiplication is cartesian.
