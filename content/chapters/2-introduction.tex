...
(n,r) &\mapsto (n1_S + f(r))
\end{align*}
gives a morphism in $\mathsf{Ring}$, so that the unit $\eta$ has the universal property required for a adjunction.
...
notion of ideally exact categories. In order to better understand the link
...
\end{align*}
Indeed, for an $R$ in $\mathsf{Ring_*}$, $\mathrm{Ker}(\pi_\mathbb{Z} \ltimes R \to \mathbb{Z})$
...
\begin{align*}
\mathbb{Z} \ltimes R &\to \mathbb{Z} \ltimes R \\
(n,r) &\mapsto (n, -n1_R + r)
\end{align*}
\begin{align*}
\mathbb{Z} \ltimes R &\to \mathbb{Z} \ltimes R \\
(n,r) &\mapsto (n, n1_R + r)
\end{align*}
are inverse each other and provide an isomorphism $(\mathbb{Z} \ltimes K, \pi_\mathbb{Z}) \cong (\mathbb{Z} \ltimes K, \pi_\mathbb{Z})$ in $(\mathsf{Ring} \downarrow \mathbb{Z})$. But $\pi_\mathbb{Z}: \mathbb{Z} \ltimes K \to \mathbb{Z}$ is also the first projection of the pullback
...
\]
is isomorphic to the pullback functor  (see Chapter 2)
\[
...
the more general notion of ideally exact category, illustrated in Chapter 5.