...
\begin{mainbox}{}
\begin{prop}
    Let $\mathcal{X}$ be a category, let $(T,\mu,\eta)$ be a monad on $\mathcal{X}$ and let $\{(A_i,\xi_i)\}_{i\in I}$ be a family of objects in $\mathcal{X}^T$. Assume that the coproducts $\sum_iA_i$ and $\sum_iT(A_i)$ exist in $\mathcal{X}$ with respective cannical injectio $\{\iota_i\}_{i\in I}$ and $\{\iota'_i\}_{i\in I}$. If the following parallel arrows in 
...
\end{tikzcd}
\end{equation}
    then both the following diagrams of parallel arrow in $\mathcal{X}$ have $p$ as $U^T$-coequalizer.
...
\begin{tikzcd}
                                                                                   & T\big(\sum_iT(A_i)\big) \arrow[rr, "{T[\ldots,T(\iota_i),\ldots]}"] &                                                        & T^2(\sum_iA_i) \arrow[rd, "\mu_{\sum_iA_i}"] &              \\
\sum_iT(A_i) \arrow[ru, "\eta_{\sum_iT(A_I)}"] \arrow[rrd, "\eta_{\sum_iT(A_I)}"'] &                                                                     & (\spadesuit)                                           &                                              & T(\sum_iA_i) \\
                                                                                   &                                                                     & T\big(\sum_iT(A_i)\big) \arrow[rru, "T(\sum_i\xi_i)"'] &                                              &             
\end{tikzcd}
\]
Consider now the top arrow in the diagram $(\spadesuit)$. Using the naturality of $\eta$, we have that
\[
...
\begin{tikzcd}
\sum_iT(A_i) \arrow[d, "{[\ldots,T(\iota_i),\ldots]}"'] \arrow[rr, "\eta_{\sum_iT(A_i)}"] &  & T\big(\sum_iT(A_i)\big) \arrow[d, "{T[\ldots,T(\iota_i),\ldots]}"] \\
T(\sum_iA_i) \arrow[rr, "\eta_{T(\sum_iA_i)}"]                                            &  & T^2(\sum_iA_i)                                                    
\end{tikzcd}
\]
\begin{align*}
    \mu_{\sum_iA_i} T[\ldots,T(\iota_i),\ldots] \eta_{\sum_iT(A_i)} &= \mu_{\sum_iA_i} \eta_{T(\sum_iA_i)} [\ldots,T(\iota_i),\ldots] \\[1.5ex]
    &= 1_{T(\sum_iA_i)} [\ldots,T(\iota_i),\ldots] \\[1.5ex]
    &= [\ldots,T(\iota_i),\ldots] 
\end{align*}
which is the top arrow in \eqref{diagr 2 coproducts}. Then, for the bottom arrow in $(\spadesuit)$, by the naturality of $\eta$, we have that
\[
% https://tikzcd.yichuanshen.de/#-E1dQ1NWIHtdiBdZ+WXSDexVb52ONJjMgfNbBRpEA
\begin{tikzcd}
\sum_iT(A_i) \arrow[rr, "\eta_{\sum_iT(A_i)}"] \arrow[d, "\xi_A+\xi_B"'] &  & T\big(\sum_iT(A_i)\big) \arrow[d, "T(\xi_A+\xi_B)"] \\
\sum_iA_i \arrow[rr, "\eta_{\sum_iA_i}"]                                 &  & T(\sum_iA_i)                                       
\end{tikzcd}
\]
