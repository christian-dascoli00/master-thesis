...
\section{Preliminary results on the E-M categories}
...
The construction of the coproducts in the Eilenberg-Moore category $\mathcal{X}^T$ will looks quite intricate at first glance and it may appear overly complicated. However, this conscrution is needed beacuse, given two object with an algebra structure each, the coprocdust of the two algebra structures is not an algebra structure in general. To make this clearer consider the following example.

\begin{rembox}{}
    \begin{ex}
        Let $\mathbf{Mon}$ be the category of monoids and let $U: \mathbf{Mon} \to \mathbf{Set}$ be the forgetful functor. One can define the free functor $F: \mathbf{Set} \to \mathbf{Mon}$ sending each set $X$ to the free monoid on $X$
        \[F(X) = \{(x_1,\ldots,x_n) \ | \ n \in \mathbb{N}, \ \forall i=1,\ldots,n, \ x_i \in X \}\]
        with the operation of concatenation. This induces a monad $T=UF$ on $\mathbf{Set}$ and the functor turns out to be monadic. The coproduct in $\mathbf{Mon}$ is the free product, while in $\mathbf{Set}$ it is the disjoint union. The free product of two monoids, as sets, is in general a bigger set than the disjoint union of the two set underlying the monoids. Thus the forgetful functor $U$ in general does not preserve coproducts. Instead, if the coproduct of the algebra structures, which correspond to the operation of the moinoids, were itself an algebra structure, the forgeftul functor would preserve coproducts.
        \[
...
\begin{tikzcd}
\mathbf{Mon} \arrow[r, "\cong"] \arrow[d, "U", shift left]     & \mathbf{Set}^T \arrow[ld, shift left] \\
\mathbf{Set} \arrow[u, "F", shift left] \arrow[ru, shift left] &                                      
\end{tikzcd}
        \]
    \end{ex}
\end{rembox}

The following will be rather intricate and technical; readers may choose to skip directly to the final outcome \Cref{algebras category has colim if the base category has coprod}. The following results are based on the work of \cite{Linton1969}.
...
\end{align*}
        ...
        and by the uniqueness of $g$, we have that $h$ is the unique morphism in $\mathcal{X}^T$ satisfying such relation.
                \[
...
\end{tikzcd}
        ...
        In order to prove that also finite coproducts exists, it suffies to do the same proof for the initial object, using \Cref{coeq for the three diagrams} again.
...
\end{tikzcd}
    ...
    Since $\mathcal{X}$ is protomodular, so is $\mathcal{X}^T$ by \Cref{algebras category is proto if the base category is proto}. Again, by mondacity, $\mathcal{A}$ inherits this properties.
    ...
    Now we aim to prove that if $m$ is a mono, then $U(m)$ acts as a mono in the moprhisms in $\mathcal{X}$ between object coming from $\mathcal{A}$, ie if $f,g : U(A) \to U(B)$ for some $A$, $B$ in $\mathcal{A}$, then $mf=mg \implies f=g$ \footnote{This will be crucial to utilize the fully functor property of the comparin functor $K$.}. First, we prove that if $m$ is a mono, then $U^T(m)$ is a mono. Let $m$ be a mono and $A$ its domain. Then $(A, 1_A, 1_A)$ is the kernel pair of $m$, in fact, given $f$ and $g$ morphisms such that $mf=mg$, we obtain the universal property by the mono.
    \[
...
\end{tikzcd}
    \]
    Since $U^T$ is a right adjoint, it preserves limit, such as kernel pairs. Thus also the follwing diagram is a kernel pair.
    \[
...
\end{tikzcd}
    \]
    This imlplies that $U^T(m)$ is a mono by the universal property of the kernel pair. 
    ...
    &\implies f = g
    \end{align*}
    Thus $K(m)$ acts as a mono in the moprhisms in $\mathcal{X}^T$ between object coming from $\mathcal{A}$. By the above discussion, $U(m)$ acts as a mono in the moprhisms in $\mathcal{X}$ between object coming from $\mathcal{A}$.

    Picking up where we left off, consider $m$ the mono arising from the (regular epi, mono) factorization of $!:0 \to 1$ in $\mathcal{A}$. Since $U$ preserves monos, then $U(m)$ is a mono. In particular, since the final obejct $1$ is a limit, it is preserved by $U$, then $U(m)$ is a mono with codomain $1$ in $\mathcal{X}$. Moreover, $\mathcal{X}$ is semiabelian, hence pointed, so $0=1$ in $\mathcal{X}$ and $U(m)$ is a mono with codomain $0$ in $\mathcal{X}$. This implies that $U(m)$ is an iso, in fact, suppose that $U(m)$ is a mono with codomaind $0$ and let $U(C)$ be its domain. Denote by $!_{U(C)}$ the unique morphism from $U(C)$ to $0$, then $!_{U(C)}$ is the inverse of $U(m)$. In particular $!_{U(C)}U(m)=!_0=1_0$. Composing with $U(m)$ we obtain $U(m)!_{U(C)}U(m)=U(m)$, thus, obsering that $U(m)!_{U(C)}$ and $1_{U(C)}$ are a morphisms between objects arising from $\mathcal{A}$, we also have that $U(m)!_{U(C)}=1_{U(C)}$. Thus $U(m)$ is an iso.
    \[
...
\end{tikzcd}
    ...
    Now we want to prove that $U$ reflects iso. As done before we prove that for the comparison functor $K$ and for $U^T$, then it follows for $U$. $K$ reflect iso beacuse, by monadicity, it is fully faithful. In fact, if $K(f):K(X) \to K(Y)$ is an iso and $g: K(Y) \to K(X)$ is its inverse in $\mathcal{X}^T$, then $K(f)g=1_{K(Y)}$ and $gK(f)=1_{K(X)}$. Let $g'$ be the morphism in $\mathcal{A}$ such that $K(g') = g$ by the fully property of $K$. So we have that
    \begin{align*}
        &1_{K(Y)} = K(f)g\\[1.5ex]
...
\end{tikzcd}
    ...
    This holds because an iso, as $m$, is also a strong epimorphism, and $e$ is a regular epimorphism, hence also a strong epimorphism; therefore, the composition $me$ is a strong epimorphism. Since the $\mathcal{A}$ is regular, then by a standard result $me$ is a regular epi, as claimed.
