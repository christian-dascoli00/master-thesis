...
The construction of the coproduct in the Eilenberg-Moore category $\mathcal{X}^T$ looks quite intricate at first glance and it may appear overly complicated. However, this conscrution is needed beacuse, given two object with an algebra structure each, the sum of the two algebra structures is not an algebra structure in general. To make this clearer consider the following example.
...
\begin{rembox}{}
    \begin{ex}
        ...
        with the operation of concatenation. This induces a monad $T=UF$ on $\mathbf{Set}$ and the functor turns out to be monadic. The coproduct in $\mathbf{Mon}$ is the free product, while in $\mathbf{Set}$ it is the disjoint union. The free product of two monoids, as a set, is in general a bigger set than the disjoint union of the two set underlying the monoids. Thus the forgetful functor $U$ in general does not preserve coproducts. Instead, if the sum of the algebra structure, which correspond to the operation of the moinoid, were itself an algebra structure, the forgeftul functor would preserve coproducts.
    \end{ex}
\end{rembox}
