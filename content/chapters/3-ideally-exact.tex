...
\begin{mainbox}{}
    \begin{prop}\label{algebras category has colim if the base category has coprod}
        Let $T:\mathcal{\mathcal{X}} \to \mathcal{X}$ be a monad. If $\mathcal{X}$ has finite coprducts and the Eilenberg-Moore category $\mathcal{X}^T$ has reflexive coequalizers, then $\mathcal{X}^T$ has finite coproducts. Moreover if $\mathcal{X}$ is also finetely cocomplete, then so is $\mathcal{X}^T$.
    \end{prop}
\end{mainbox}
...
We are now ready to prove the theorem of characterization, which will lead to the definition of ideally exact categories.
