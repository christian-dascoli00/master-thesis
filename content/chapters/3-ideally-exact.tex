...
\]
        ...
        \[ f=mq \implies f \xi_A = mq\xi_A \implies \xi_B T(f) = m \xi_Q T(q) \implies \]
...
\[
% https://tikzcd.yichuanshen.de/#N4Igdg9gJgpgziAXAbVABwnAlgFyxMJZABgBpiBdUkANwEMAbAVxiRAB12BbOnACwDGjYAA0AvgD0AKiDGl0mXPkIoyARiq1GLNpx78hDUWNnyQGbHgJEATOU31mrRCAAUI0pwAeWAJSmFS2UiAGZ7akcdFxEA80UrFWQ7DQjtZxAYuUClaxQwlK0nNlcpd19PbiYAfRF-MU0YKABzeCJQADMAJwguJDIQHAgkNWo4Pix2nCQAWhHCqJAAVWlYrp7h6kG+0fHJmbnI9IAxFayQNd7EOwGhxDCBuiwGNh40OC3UopdlmTOLpAALJtbgBWTaPZ4uV7vIafBYnX4UMRAA
\begin{tikzcd}
\mathcal{X}^T \arrow[d, "U^T", shift left] &  & {(X,\xi)} \arrow[r, "U^T", maps to] & X              \\
\mathcal{X} \arrow[u, "F^T", shift left]   &  & X \arrow[r, "F^T", maps to]         & {(T(X),\mu_X)}
\end{tikzcd}
\]
...
\begin{mainbox}{}
    \begin{lemma}\label{U-coeq}
        ...
        \[
        % https://tikzcd.yichuanshen.de/#N4Igdg9gJgpgziAXAbVABwnAlgFyxMJZABgBpiBdUkANwEMAbAVxiRAEEB9LEAX1PSZc+QigBM5KrUYs2AVQBiACi5YAlHwEgM2PASISxU+s1aIQcpQA0N--5VQFLEFg7qQ9SXED4wkksn+ECU6m0xEjD4AZkOPxcIBu6PhWLY5x4dOlSDljMVypZXzZIBxPDxBKJpIaojFVJpu21iCCjLKSthGOeavMwNB4N5Dv2TvlSFdBoxRqS3s5vtNO3NQqtMTY4vtWsDiqOiAA7F7wgBHOr012ZgAcubzHV4QA
\begin{tikzcd}
A_i \arrow[rr, "\eta_{A_i}"] \arrow[rrdd, "f_i", shift left] \arrow[rrdd, "g_i"', shift right] &  & UF(A_i) \arrow[dd, "U(\bar{f}_i)", shift left] \arrow[dd, "U(\bar{g}_i)"', shift right] &  & F(A_i) \arrow[r, "\bar{f}_i", shift left] \arrow[r, "\bar{g_i}"', shift right] & X \arrow[r, "q"]       & Q    \\
                                                                                               &  &                                                                                         &  &                                                                                &                        &      \\
                                                                                               &  & U(X)                                                                                    &  & A_i \arrow[r, "f_i", shift left] \arrow[r, "g_i"', shift right]                & U(X) \arrow[r, "U(q)"] & U(Q)
\end{tikzcd}
        \]
    \end{lemma}
\end{mainbox}
...
\begin{mainbox}{}
\begin{prop}
    Let $\mathcal{X}$ be a category, let $(T,\mu,\eta)$ be a monad on $\mathcal{X}$ and let $\{(A_i,\xi_i)\}_{i\in I}$ be a family of objects in $\mathcal{X}^T$. Assume that the coproducts $\sum_iA_i$ and $\sum_iT(A_i)$ exist. If the following parallel arrows in 
    $\mathcal{X}^T$ admits a coequalizer $q:T(\sum_i)\to (Q,\xi_Q)$
    \[
...
\begin{tikzcd}
                                                                                                                  & {\Big(T^2(\sum_iA_i),\mu\Big)} \arrow[rd, "\mu_{\sum_iA_i}"] &                              \\
{\Big(T\big(\sum_iT(A_i)\big),\mu\Big)} \arrow[rr, "T(\sum_i\xi_i)"'] \arrow[ru, "{T[\ldots,T(\iota_i),\ldots]}"] &                                                              & {\Big(T(\sum_iA_i),\mu\Big)}
\end{tikzcd}
    \]
    then both the folowing diagrams of parallel arrow in $\mathcal{X}$ have $p$ as $U^T$-coequalizer.
    \[
    % https://tikzcd.yichuanshen.de/#
\begin{tikzcd}
T(A_i) \arrow[rrr, "T(\iota_i)"] \arrow[rd, "\xi_{i}"'] &                          &                                            & T(\sum_i A_i) \\
                                                        & A_i \arrow[r, "\iota_i"] & \sum_iA_i \arrow[ru, "\eta_{\sum_i A_i}"'] &              
\end{tikzcd}
    \]
    \[
...
\begin{tikzcd}
\sum_iT(A_i) \arrow[rr, "{[\ldots,T(\iota_i),\ldots]}"] \arrow[rd, "\sum_i \xi_i"'] &                                           & T(\sum_iA_i) \\
                                                                                    & \sum_iA_i \arrow[ru, "\eta_{\sum_iA_i}"'] &             
\end{tikzcd}
    \]
...
\begin{tikzcd}
                                                                                                                                          &  & T^3(\sum_iA_i) \arrow[dd, "\mu_{T(\sum_iA_i)}", pos=0.8] \arrow[rrd, "T\big(\mu_{\sum_iA_i}\big)"] &  &                                              \\
T^2\big(\sum_iT(A_i)\big) \arrow[dd, "\mu_{\sum_iT(A_i)}"'] \arrow[rru, "{T^2[\ldots,T(\iota_i),\ldots]}"] \arrow[rrrr, "T(\sum_i\xi_i)", pos=0.3, crossing over] &  &                                                                                   &  & T^2(\sum_iA_i) \arrow[dd, "\mu_{\sum_iA_i}"] \\
                                                                                                                                          &  & T^2(\sum_iA_i) \arrow[rrd, "\mu_{\sum_iA_i}"]                                     &  &                                              \\
T\big(\sum_iT(A_i)\big) \arrow[rrrr, "T(\sum_i\xi_i)"'] \arrow[rru, "{T[\ldots,T(\iota_i),\ldots]}"]                                      &  &                                                                                   &  & T(\sum_iA_i)                                
\end{tikzcd}
\]
    \end{proof}
\end{subbox}
...
T\bigl( T(A)+T(B)\bigr) \arrow[rr, "{T[\xi_A,\xi_B]}"', shift right] \arrow[ru, "{T[T(\iota_1),T(\iota_2)]}", pos=0.65]                                       &                                                                 & T(A+B) \arrow[ll, "T(\eta_A+\eta_B)" description, shift right] \arrow[rr, "q"]                      &  & Q                        
\end{tikzcd}
\]
...
\]
\end{minipage}
        We still need to prove that they are morphisms in $\mathcal{X}^T$. We prove the first one, the second one is similar. Note that $\iota_1$ and $\eta_{A+B}$ are not a morphism in $\mathcal{X}^T$, this is not a problem, since we are going to prove that the whole composition $q\eta_{A+B}\iota_1$ is a morphism in $\mathcal{X}^T$. Consider the following diagram. All diagrams commute except the left and the external ones. We aim to prove that the left diagram commutes, so that the external one commutes as well.
        \[
