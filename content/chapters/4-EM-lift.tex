\chapter{Lifting results on the E-M categories}
...
\]
        ...
        \end{align*}
        We know that $T(e)$ is a regular epi, thus it is a epi. So we can cancel it from both sides and we obtain the condition for $m$ to be a morphism in $\mathcal{X}^T$:
        \[ \xi_B T(m) =  m \xi_Q \]
...
In this section we aim construct products and coproducts in the Eilenberg–Moore category. Products are quite easy to lift, and we implicitly already saw it in the exactness lift, whereas coproducts need additional conditions.
...
\end{tikzcd}
\]
The algebra structure coincides with the one given by the creations of limits by $U^T$, in fact, in that case the algebra structure is the unique making the diagram commutes. 
...
The construction of the coproducts in the Eilenberg-Moore category $\mathcal{X}^T$ will looks quite intricate at first glance and it may appear overly complicated. However, this construction is needed because, given two object with an algebra structure each, the coproduct of the two algebra structures is not an algebra structure in general. To make this clearer consider the following example.
...
The following gives a criteria to construct coproducts in the Eilenberg–Moore category, assuming they exist in the base category. This is possible if we assume the existence of reflexive coequalizers in the Eilenberg–Moore category.
...
\begin{tikzcd}
T^2(A+B) \arrow[dd, "\mu_{A+B}"] \arrow[rr, "{T[f_1,f_2]}"] &  & T^2(K) \arrow[dd, "\mu_K"] \arrow[rr, "\mu_K"] &  & T(K) \arrow[dd, "\xi_K"] \\
                                                            &  &                                                &  &                          \\
