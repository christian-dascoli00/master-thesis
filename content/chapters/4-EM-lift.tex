\chapter{Lifting results on E-M categories}

\fontsize{12}{14.5}\selectfont

\section{Exactness and protomodularity lift}

We present here some technical results on the Eilenberg-Moore categories. These will play a key role in the definition and characterization of the ideally exact categories.

\begin{mainbox}{}
    \begin{prop}\label{algebras category is exact if the base category is exact}
        Let $T$ be a monad preserving regular epimorphisms and kernel pairs. If $\mathcal{X}$ is exact, then the Eilenberg-Moore category $\mathcal{X}^T$ is exact.
    \end{prop}
\end{mainbox}

\begin{subbox}{}
    \begin{proof}
        Assume $\mathcal{X}$ is exact. With the given hypotheses on the monad $T$, we aim to prove that $\mathcal{X}^T$ is exact. We need to establish that it is finitely complete, that each equivalence relation is the kernel pair of some arrow, that regular epis are pullback stable and that each morphism admits a (regular epi, mono) factorization.
        ...
        We proceed to prove that each equivalent relation is the kernel pair of some arrow. Consider an equivalence relation $R$ in $\mathcal{X}^T$ with $\xi_R$ and $\xi_A$ giving the structure of algebras, as in the diagram below. Then, under the forgetful functor $U^T$, $R$ is an equivalence relation in $\mathcal{X}$. $\mathcal{X}$ is exact, so $R$ is the kernel pair of some arrow. In particular, by \Cref{equiv rel is kernel pair of the coeq}, $R$ is the kernel pair of its coequalizer. Let us call it $q:A \to C$. Since $T$ preserves kernel pairs, we have that $T(R)$ is the kernel pair of $T(q)$. Moreover $T$ preserves regular epis, so $T(q)$ is a regular epi. By \Cref{coeq of the kernel pair}, $T(q)$ is the coequalizer of its kernel pair $(T(R),T(r_1),T(r_2))$. Since $q\xi_AT(r_1)= qr_1\xi_R = qr_2\xi_R=q\xi_AT(r_2)$, by the universal property of the coequalizer, there exists a unique morphism $\xi_C$ such that $\xi_CT(q) = q\xi_A$. The morphism $\xi_C$ is the algebra structure of $T(B)$. Since $U^T$ creates all the limits that $\mathcal{X}$ has, we can conclude that $R$ is the kernel pair in $\mathcal{X}^T$ of the arrow $q$.
        \[
% https://tikzcd.yichuanshen.de/#N4Igdg9gJgpgziAXAbVABwnAlgFyxMJZABgBoAmAXVJADcBDAGwFcYkQAlEAX1PU1z5CKchWp0mrdgEEefEBmx4CRACxiaDFm0QgAwnP5KhRMsXFapugCoAKDgEpDCgcuHJR5zZJ0g70p14jQRUUdS8JbXY7PUDxGCgAc3giUAAzACcIAFskAEYaHAgkURAACxh6KHYcAHcICqqEbyjdAEdnTJykMhAi-Jo4Mqw0nCQAWgLIqxAMgH08zqzcxF7+xCmhkbGNlpn58hAaRnoAIxhGAAVXE10MrESysaCQLpWAZkLi1b3fAB0-gAPLBzLjHLBgXxwCCMLDVF5vJDqPrfKaWf5AkGyBHLJFfJAAVl+0VsbScNEa1V0dQalSgCBx3UQRJRJWJugBwLmBnBkPYUHoQwSSyZn1ZiGRW1GEzRPhJ8zygXkiMQYvWkuG0t2IBO5yuN1CsweTyO018dgOcW4QA
\begin{tikzcd}
T(R) \arrow[dd, "\xi_R"] \arrow[rr, "T(r_1)", shift left] \arrow[rr, "T(r_2)"', shift right] &  & T(A) \arrow[dd, "\xi_A"] \arrow[rr, "T(q)", two heads] &  & T(C) \arrow[dd, "\xi_C", dashed] \\
                                                                                             &  &                                                        &  &                                  \\
R \arrow[rr, "r_1", shift left] \arrow[rr, "r_2"', shift right]                              &  & A \arrow[rr, "q", two heads]                           &  & C                               
\end{tikzcd}
    \]

        Consider now a morphism $f:(A,\xi_A)\to (B, \xi_B)$ in $\mathcal{X}^T$, we aim to prove that it admits a (regular epi, mono) factorization. The morphism $f$, under the forgetful functor $U^T$, is a morphism in $\mathcal{X}$, which is exact and in particular regular. Thus $f$ admits a (regular epi, mono) factorization in $\mathcal{X}$. Let it be $f=me$ with $m:Q \to B$ mono and $e: A \to Q$ regular epi, thus the coequalizer of some pair of parallel arrows. $A$ and $B$ already have an algebra structure. We want to provide $Q$ and the domain of the parallel arrows with a structure of algebra, so that we can lift the factorization to $\mathcal{X}^T$.
        \[
% https://tikzcd.yichuanshen.de/#N4Igdg9gJgpgziAXAbVABwnAlgFyxMJZAJgBpiBdUkANwEMAbAVxiRAEEQBfU9TXfIRQAWclVqMWbAELdeIDNjwEiAZlKrx9Zq0QgAinL5LBRMgAYtk3SAAqACnYBKIwv7KhyUZerapeh2kXHmMBFRRzMV9rNgANV0Uwz0ifCR02B1jg+USPNVIARit0gPt9YPEYKABzeCJQADMAJwgAWyRIkBwIJDI0-xBWagY6ACMYBgAFd1M9JqxqgAscEGpFmDooNhwAdwh1zYQQkGa23upupAKLuiwGbdv76JKQduGxiemTcJB5pZXjqd2ohOpdENd+jYGq4gUh1F0eiDngMADoogAeWAA+pxAS1gaIEVdkTY0ZisbI8WdEPCwYS-DYHA1so18UgAKwXRGdOCLLANFbgqnAgBsXLh1F5-MFBWFSDFRJpkr5AqQAFpZfJYYhOYqeSrBRq5UrFQB2EkZewwFzvcZTGY-P7LGFsxDmxX0mKlVo2kAjO1fJJsJ0AihcIA
\begin{tikzcd}
T(X) \arrow[rr, shift right] \arrow[rr, shift left] &  & T(A) \arrow[dd, "\xi_A"] \arrow[rr, "T(f)"] \arrow[rd, "T(e)"'] &                          & T(B) \arrow[dd, "\xi_B"] \\
                                                    &  &                                                                 & T(Q) \arrow[ru, "T(m)"'] &                          \\
X \arrow[rr, shift right] \arrow[rr, shift left]    &  & A \arrow[rd, "e"', two heads] \arrow[rr, "f"]                   &                          & B                        \\
                                                    &  &                                                                 & Q \arrow[ru, "m"', tail] &                         
\end{tikzcd}
        \]
...
\]
        ...
        In order to prove the factorization of $f$ in $\mathcal{X}^T$, it remains to establish that $m$ is a morphism in $\mathcal{X}^T$ and that it is a mono. Observe that $m$ is a mono in $\mathcal{X^T}$; in fact it is a mono in $\mathcal{X}$ and, furthermore, it is merely a morphism in $\mathcal{X}$ that satisfies the algebra condition. We still need to establish that $m$ is a morphism in $\mathcal{X}^T$.
        \begin{align*}
            f=me &\implies f \xi_A = me\xi_A \\[1.5ex]
            &\implies \xi_B T(f) = m \xi_Q T(e) \\[1.5ex]
            &\implies \xi_B T(m) T(e) =  m \xi_Q T(e)
        \end{align*}
        We know that $T(e)$ is a regular epi, thus it is an epi. So we can cancel it from both sides and we obtain the condition for $m$ to be a morphism in $\mathcal{X}^T$:
        \[ \xi_B T(m) =  m \xi_Q \]
...
\begin{subbox}{}
    \begin{proof}
        Since the forgetful functor $\mathcal{X}^T \to \mathcal{X}$ creates all the limits that $\mathcal{X}$ has, we can assert that $\mathcal{X}^T$ is finitely complete.

        Then, let $f:(X,\xi) \to (Y,\xi')$ be a morphism in $\mathcal{X}^T$. The following diagram of functors is commutative. This essentially follows from the fact that the forgetful functor $\mathcal{X}^T \to \mathcal{X}$ creates all the limits that $\mathcal{X}$ has, in particular the pullbacks, which are used to define the change-of-base functor $f^*$.
        \[
...
\section{Products and coproducts lift}

In this section we aim to construct products and coproducts in the Eilenberg–Moore category. Products are quite easy to lift, and we implicitly already saw it in the exactness lift, whereas coproducts need additional conditions.
 
Even if we already know that products can be easily lifted, since the forgetful functor $U^T$ creates all the limits, it will be useful to have the explicit construction.

\begin{rembox}{}
    \begin{rem}\label{product lift}
        Given $A$, $B$ objects in a category with products and and $T$ a monad on such category, we can construct the product in the Eilenberg-Moore category $\mathcal{X}^T$ as follows. Let $\pi_1:A\times B \to A$ and $\pi_2:A\times B \to B$ be the projections of the product in $\mathcal{X}$, then we define the product in $\mathcal{X}^T$ as follows.
\[
% https://tikzcd.yichuanshen.de/#N4Igdg9gJgpgziAXAbVABwnAlgFyxMJZABgBpiBdUkANwEMAbAVxiRABUAKAQQEoQAvqXSZc+QigBM5KrUYs2XbgB1leALbwABACF+QkdjwEiAFhnV6zVog6c9g4SAxHxZ0pNlWFtnY8NiJihknpbyNiDc-
\begin{tikzcd}
T(A) \arrow[dd, "\xi_A"] &  & T(A\times B) \arrow[d, "{\langle T(\pi_1), T(\pi_2)\rangle}"] \arrow[rr, "T(\pi_2)"] \arrow[ll, "T(\pi_1)"'] &  & T(B) \arrow[dd, "\xi_B"] \\
                         &  & T(A)\times T(B) \arrow[d, "\xi_A \times \xi_B"]                                                              &  &                          \\
A                        &  & A\times B \arrow[rr, "\pi_2"] \arrow[ll, "\pi_1"']                                                           &  & B                       
\end{tikzcd}
\]
The algebra structure coincides with the one given by the creations of limits by $U^T$, in fact, in that case the algebra structure is the unique making the diagram commute. 
The universal property holds by the universal property of the product in $\mathcal{X}$, in fact one can prove that the unique morphism induced by the product in $\mathcal{X}$ is also a morphism in $\mathcal{X}^T$.
\end{rem}
\end{rembox}
...
\begin{mainbox}{}
    \begin{prop}
        Let $T:\mathcal{\mathcal{X}} \to \mathcal{X}$ be a monad. If $\mathcal{X}$ has finite products, then $\mathcal{X}^T$ has finite products.
    \end{prop}
\end{mainbox}

We now proceed with results that will allow us to construct coproducts in the Eilenberg–Moore category, assuming they exist in the base category.

The construction of the coproducts in the Eilenberg-Moore category $\mathcal{X}^T$ will look quite intricate at first glance and it may appear overly complicated. However, this construction is needed because, given two object with an algebra structure each, the coproduct of the two algebra structures is not an algebra structure in general. To make this clearer consider the following example.

\begin{rembox}{}
    \begin{ex}
        Let $\mathsf{Mon}$ be the category of monoids and let $U: \mathsf{Mon} \to \mathsf{Set}$ be the forgetful functor. One can define the free functor $F: \mathsf{Set} \to \mathsf{Mon}$ sending each set $X$ to the free monoid on $X$
        \[F(X) = \{(x_1,\ldots,x_n) \ | \ n \in \mathbb{N}, \ \forall i=1,\ldots,n, \ x_i \in X \}\]
        with the operation of concatenation. This induces a monad $T=UF$ on $\mathsf{Set}$ and the functor turns out to be monadic. The coproduct in $\mathsf{Mon}$ is the free product, while in $\mathsf{Set}$ it is the disjoint union. The free product of two monoids, as sets, is in general a bigger set than the disjoint union of the two set underlying the monoids. Thus the forgetful functor $U$ in general does not preserve coproducts. Instead, if the coproduct of the algebra structures, which correspond to the operation of the monoids, were itself an algebra structure, the forgetful functor would preserve coproducts.
        \[
        % https://tikzcd.yichuanshen.de/#
\begin{tikzcd}
\mathsf{Mon} \arrow[r, "\cong"] \arrow[d, "U", shift left]     & \mathsf{Set}^T \arrow[ld, shift left] \\
\mathsf{Set} \arrow[u, "F", shift left] \arrow[ru, shift left] &                                      
\end{tikzcd}
        \]
    \end{ex}
\end{rembox}

The following will be rather intricate and technical; we aim to get to the final outcome \Cref{algebras category has colim if the base category has coprod}. The following results are based on the work of \cite{Linton1969}.

For the sake of clarity, we first present a technical definition.

\begin{mainbox}{}
    \begin{defi}
        Let $U:\mathcal{X} \to \mathcal{A}$ be a functor, let $X$ be an object in $\mathcal{X}$ and let $\{(f_i,g_i)\}_{i\in I}$ be a family of pairs of parallel arrows in $\mathcal{A}$, where $f_i,g_i: A_i \to U(X)$. A morphism $q:X \to Q$ in $\mathcal{X}$ is said to be a $U$\textbf{-coequalizer} of $\{(f_i,g_i)\}_{i\in I}$ if 
        \begin{itemize}
...
Before continuing, we recall the universal property of the unit in an adjunction. Consider a functor $U: \mathcal{A} \to \mathcal{B}$ right adjoint to a functor $F$. Then for each object $A$ in $\mathcal{A}$, $B$ in $\mathcal{B}$, and $f:B \to U(A)$ in $\mathcal{B}$ there exists a unique morphism $g: F(B) \to A$ in $\mathcal{A}$ such that $f=U(g)\eta_B$. We denote $g = \bar{f}$. 
\[
...
\begin{mainbox}{}
    \begin{lemma}\label{U-coeq}
        Let $U:\mathcal{X} \to \mathcal{A}$ be a right adjoint functor to a functor $F:\mathcal{A} \to \mathcal{X}$, let $X$ be an object in $\mathcal{X}$ and let $\{(f_i,g_i)\}_{i\in I}$ be a family of parallel arrows in $\mathcal{X}$, where $f_i,g_i: A_i \to U(X)$. Consider the family $\{(\bar{f}_i,\bar{g}_i)\}_{i\in I}$ of unique morphisms induced by the universal property of the unit, such that $f_i=U(\bar{f}_i)\eta_{A_i}$ and $g_i=U(\bar{g}_i)\eta_{A_i}$. If $q:X \to Q$ is the coequalizer of the family $\{(\bar{f}_i,\bar{g}_i)\}_{i\in I}$ in $\mathcal{X}$, then $q$ is the $U$-coequalizer of the family $\{(f_i,g_i)\}_{i\in I}$ in $\mathcal{A}$.
...
\begin{subbox}{}
\begin{proof}
    Assume $q:X \to Q$ is the coequalizer of the family $\{(\bar{f}_i,\bar{g}_i)\}_{i\in I}$ in $\mathcal{X}$, we want to prove that $q$ is the $U$-coequalizer of the family $\{(f_i,g_i)\}_{i\in I}$ in $\mathcal{A}$. First we show, for each $i$, that $U(q)f_i=U(q)g_i$. Assume that $q\bar{f}_i=q\bar{g}_i$.
    \begin{align*}
        q\bar{f}_i=q\bar{g}_i &\implies U(q) U(\bar{f}_i) = U(q) U(\bar{g}_i) \\[1.5ex]
        &\implies U(q) U(\bar{f}_i) \eta_{A_i}= U(q) U(\bar{g}_i) \eta_{A_i} \\[1.5ex]
        &\implies U(q)f_i=U(q)g_i
    \end{align*}

    We now aim to prove the universal property of the $U$-coequalizer. Let $h:X \to Y$ be a morphism such that $U(h)f_i=U(h)g_i$. By the universal property of the unit applied to $U(h)f_i=U(h)g_i : A_i \to U(Y)$ there exist a unique morphism $k:F(A_i) \to Y$ such that $U(k) \eta_{A_i}=U(h)f_i=U(h)g_i$. Both $h\bar{f}_i$ and
...
\end{tikzcd}
    \]
    Then, by the universal property of the coequalizer $q$ in $\mathcal{X}$, there exists a unique morphism $t:Q \to Y$ such that $tq=h$.
\end{proof}
\end{subbox}
...
\begin{prop}\label{coeq for the three diagrams}
    ...
    $\mathcal{X}^T$ admit a coequalizer $q:T(\amalg_i)\to (Q,\xi_Q)$
...
\end{tikzcd}
\end{equation}
    then both the following diagrams of parallel arrow in $\mathcal{X}$ have $q$ as $U^T$-coequalizer $\forall j \in I$.
...
\begin{subbox}{}
    \begin{proof}
        Consider the diagram \eqref{diagr 1 coproducts} of parallel arrows in $\mathcal{X}^T$, and $q$ its coequalizer, here below explicitly.
        \[
...
\]
which is the bottom arrow in \eqref{diagr 2 coproducts}. By \Cref{U-coeq}, it follows that the diagram \eqref{diagr 2 coproducts} has $q$ as $U^T$-coequalizer.

It remains to prove that the diagram \eqref{diagr 3 coproducts} has $q$ as $U^T$-coequalizer. We already know that \eqref{diagr 2 coproducts} has $q$ as $U^T$-coequalizer. Note that these two diagrams are related by the canonical injection $\iota'_j:T(A_j) \to \amalg_iT(A_i)$, as shown in the diagram below. In particular, by the universal property of the coproduct, it holds that $(\amalg_i \xi_i) \iota'_j = \iota_j\xi_j$ and $[\ldots,T(\iota_i),\ldots] \iota'_j = T(\iota_j)$.
\[
...
The universal property of the $U^T$-coequalizer of \eqref{diagr 3 coproducts} also holds. In fact, let $h$ be a morphism in $\mathcal{X}^T$ such that $U^T(h) T(\iota_j) = U^T(h) \iota_j \xi_j \ \ \forall j \in I$, then 
\begin{align*}
...
The following gives a criterion to construct coproducts in the Eilenberg–Moore category, assuming they exist in the base category. This is possible if we assume the existence of reflexive coequalizers in the Eilenberg–Moore category.

\begin{mainbox}{}
    \begin{prop}\label{algebras category has colim if the base category has coprod}
        Let $(T,\mu,\eta)$ be a monad on $\mathcal{X}$. If $\mathcal{X}$ has finite coproducts and the Eilenberg-Moore category $\mathcal{X}^T$ has reflexive coequalizers, then $\mathcal{X}^T$ has finite coproducts. Moreover $\mathcal{X}^T$ is finitely cocomplete.
    \end{prop}
\end{mainbox}

\begin{subbox}{}
    \begin{proof}
        Let $(T,\mu,\eta)$ be the given monad, and suppose that $\mathcal{X}$ has finite coproducts. Let $(A,\xi_A)$ and $(B,\xi_B)$ be two objects in $\mathcal{X}^T$. First, we aim to prove that $(A,\xi_A)$ and $(B,\xi_B)$ have a coproduct in $\mathcal{X}^T$. All the coproducts in the diagram below are taken in $\mathcal{X}$, and therefore they exist. Let $\iota_1$ and $\iota_2$ be the canonical injections in $\mathcal{X}$ of $A$ and $B$ into $A+B$, respectively. Our goal is to prove that the diagram below is composed of a reflexive pair in $\mathcal{X}^T$. Then, by assumption, it admits a coequalizer in $\mathcal{X}^T$, denoted by $q$. This diagram correspond to the diagram \eqref{diagr 1 coproducts} in \Cref{coeq for the three diagrams}.
            \[
...
\]
        ...
        \end{itemize}
        We proceed to show that they form a reflexive pair by exhibiting a common section. $T(\eta_A+\eta_B)$ is such a common section, as shown below. It follows the first composition
        ...
        Let $q:T(A+B) \to Q$ be the coequalizer in $\mathcal{X}$, and $\xi_Q$ be the algebra structure of $Q$. We aim to prove that $(Q,\xi_Q)$ is the coproduct of $(A,\xi_A)$ and $(B,\xi_B)$ in $\mathcal{X}^T$. First we need to exhibit the canonical injections $\iota_{Q,1}$ and $\iota_{Q,2}$. They are respectively defined as the following compositions.
...
% As the one above 
\begin{tikzcd}
B \arrow[rr, "\iota_2"] &  & A+B \arrow[rr, "\eta_{A+B}"] &  & T(A+B) \arrow[rr, "q"] &  & Q
\end{tikzcd}
\]
\end{minipage}
        We still need to prove that they are morphisms. Note that $\iota_1$ and $\eta_{A+B}$ are not morphisms in $\mathcal{X}^T$. This is not a problem, however, since we will show that the whole composition $q\eta_{A+B}\iota_1$ is a morphism in $\mathcal{X}^T$ in $\mathcal{X}^T$. We prove the first one, the second one is similar. Consider the following diagram. All diagrams, except the left and the external ones, commute. We aim to prove that the left diagram commutes, so that the external one commutes as well. This holds by \Cref{coeq for the three diagrams}, since it corresponds to the diagram \eqref{diagr 3 coproducts}.
        \[
...
\end{tikzcd}
        ...
        Once we have the canonical injections, we need to show that they satisfy the universal property of coproducts in $\mathcal{X}^T$. Let $f_i:(A_i,\xi_i) \to (K,\xi_K)$ for $i=1,2$ in $\mathcal{X}^T$. We aim to prove that there exists a unique morphism $h:(Q,\xi_Q) \to (K,\xi_K)$ in $\mathcal{X}^T$ such that $h\iota_{Q,i} = f_i$ for $i=1,2$. Refer to the diagram at the end of the proof for a visualization of the following arguments.

        First, we show that there exists a unique morphism $g:(T(A+B),\mu_{A+B}) \to (K,\xi_K)$ in $\mathcal{X}^T$ such that $g(\eta_{A+B})\iota_i=f_i$ for $i=1,2$. Define $g=\xi_K T[f_1,f_2]$. It is a morphism in $\mathcal{X}^T$, in fact the following diagram commutes.
        \[
        % https://tikzcd.yichuanshen.de/#-U6gAPLBXOYHFb9JD-M6AkFg0SUURAA
\begin{tikzcd}
T^2(A+B) \arrow[dd, "\mu_{A+B}"] \arrow[rr, "{T^2[f_1,f_2]}"] &  & T^2(K) \arrow[dd, "\mu_K"] \arrow[rr, "T(\xi_K)"] &  & T(K) \arrow[dd, "\xi_K"] \\
                                                            &  &                                                &  &                          \\
T(A+B) \arrow[rr, "{T[f_1,f_2]}"]                           &  & T(K) \arrow[rr, "\xi_K"]                       &  & K                       
\end{tikzcd}
        \]
        Suppose now that there exists an other morphism $g':(T(A+B),\mu_{A+B}) \to (K,\xi_K)$ such that $g'(\eta_{A+B})\iota_i=f_i$ for $i=1,2$. Then we have that
        \begin{align*}
            g&(\eta_{A+B})\iota_i = g'(\eta_{A+B})\iota_i \ \ i=1,2 \\[1.5ex]
            &\implies g(\eta_{A+B}) = g'(\eta_{A+B}) \\[1.5ex]
            &\implies U^T(g)(\eta_{A+B}) = U^T(g')(\eta_{A+B}) \\[1.5ex]
            &\implies g=g'
        \end{align*}
        where we used the uniqueness in the universal property of the unit and the fact that $g$ and $g'$ are morphisms in $\mathcal{X}^T$. So $g$ is unique. Moreover, we have that $gT(\iota_i) = g (\eta_{A+B}) \iota_i \xi_i$ for $i=1,2$. In fact, we have that
        \[
% https://tikzcd.yichuanshen.de/#-gAPLDbFJeECveaINZLFaIX7OEKAkFgl4zaFkOEfdYhbg7cjVKFILHfRAAdlxFS4gPwOHouyJGKQlOxiE+fzxW12pH2lHRb0QpPhOk57BRoOUEOJiFFZIaVOMtIg9N2hxAxzOl2uvhAd0ez2lzMQCrZiyR4v+MFVwHcgnVmvOV1qkj19yeAuhcvhAA4jYLvUgAJyKkCA60MqWqGUhtk+0MS7bKI6nJ06136j3+6Hxtm-M5gKAk0MEh2p7Uu9iZw3R42xsmwi15AlMwX1+Hm8p5ZWqwkDQRAA
\begin{tikzcd}
                                                                         & T(A+B) \arrow[rrd, "{T[f_1,f_2]}"] &  &                                                                          &  &                           \\
T(A) \arrow[rrr, "T(f_1)"] \arrow[ru, "T(\iota_1)"] \arrow[ddd, "\xi_A"] &                                    &  & T(K) \arrow[rr, "\xi_K"]                                                 &  & K                         \\
                                                                         & A \arrow[rrrru, "f_1"']            &  &                                                                          &  &                           \\
                                                                         &                                    &  & T(A) \arrow[llu, "\xi_A"'] \arrow[rrd, "T(f_1)"] \arrow[d, "T(\iota_1)"] &  &                           \\
A \arrow[r, "\iota_1"'] \arrow[ruu] \arrow[rrru, "\eta_A"]               & A+B \arrow[rr, "\eta_{A+B}"']      &  & T(A+B) \arrow[rr, "{T[f_1,f_2]}"']                                       &  & T(K) \arrow[uuu, "\xi_K"]
\end{tikzcd}
\]
\begin{align*}
    gT(\iota_1) &= \xi_K T[f_1,f_2] T(\iota_i) = \xi_K T([f_1,f_2]\iota_i) \\[1.5ex]
    &= \xi_K T(f_1) = f_1 \xi_A = f_1 1_A \xi_A \\[1.5ex]
    &= f_1 \xi_A \eta_{A} \xi_A = \xi_K T(f_1) \eta_{A} \xi_A \\[1.5ex]
    &= \xi_K T[f_1,f_2] T(\iota_1) \eta_{A} \xi_A \\[1.5ex]
    &= \xi_K T[f_1,f_2] (\eta_{A+B}) \iota_1 \xi_A \\[1.5ex]
    &= g (\eta_{A+B}) \iota_1 \xi_A
\end{align*}
        and the same holds for $B$. Therefore $g$ has equal compositions with the parallel arrows corresponding to \eqref{diagr 3 coproducts}, which has, by \Cref{coeq for the three diagrams}, $q$ as $U^T$-coequalizer. Then by the universal property of the $U^T$-coequalizer, it follows that there exists a unique morphism $h:(Q,\xi_Q) \to (K,\xi_K)$ such that $g=h q$ in $\mathcal{X}^T$. Thus, we have that
        \[ h\iota_{Q,i} = h q (\eta_{A+B}) \iota_i = g (\eta_{A+B}) \iota_i = f_i \ \ \ i=1,2 \]
        and by the uniqueness of $g$, we have that $h$ is the unique morphism in $\mathcal{X}^T$ satisfying such condition.
                \[
% https://tikzcd.yichuanshen.de/#--5+blby6w65t8lTPnLbbZ640KvXL3nW1BfouP4PiA0Ibs4Nwfr+nBfMIIEwI2yr2kSk7lpYlZDoq17OHywEQV8PwgA6PZTthM6kaBbavjSMFjgRZ4FniWz0Q8sEMiqHZcHGIaJgGQb6j63hIWRGFlr05JutBhEMsuGz8Qm-rJqmbzeKR5GYdJOHbCIQA
\begin{tikzcd}
                                                                                                     &  & T(Q) \arrow[dd, "\xi_Q"]  &                                                                    &                                                &                          & T(B)  \arrow[dd, "\xi_B"] \arrow[llld, "T(\eta_{A+B}\iota_2)"'] \\
                                                                                                     &  &                                                             & T^2(A+B) \arrow[rd, "{T^2[f_1,f_2]}"] \arrow[lu, "T(q)"'] &                                                &                          &                                                                                                   \\
T(A)  \arrow[dd, "\xi_A"'] &  & Q                            &                                                                    & T^2(K)  &                          & B \arrow[lddd, "f_2"', bend left=20] \arrow[llld, "\eta_{A+B}\iota_2"', pos=0.25]                              \\
                                                                                                     &  &                                                             & T(A+B) \arrow[rd, "{T[f_1,f_2]}", pos=0.25] \arrow[lu, "q"']                 &                                                & T(K) \arrow[dd, "\xi_K"] &                                                                                                   \\
A \arrow[rrru, "\eta_{A+B}\iota_1"', pos=0.3] \arrow[rrrrrd, "f_1", bend right=25, pos=0.2]                               &  &                                                             &                                                                    & T(K) \arrow[rd, "\xi_K"']                      &                          &                                                                                                   \\
                                                                                                     &  &                                                             &                                                                    &                                                & K                        &                                                                                       
\arrow[from=3-1, to=2-4, "T(\eta_{A+B}\iota_1)", crossing over]
\arrow[from=3-5, to=5-5, "\mu_K"', crossing over, pos=0.1] 
\arrow[from=3-5, to=4-6, "\mu_K", crossing over, pos=0.2]
\arrow[from=1-3, to=4-6, "T(h)", bend right, crossing over]
\arrow[from=3-3, to=6-6, "h", bend right, crossing over]
\arrow[from=3-1, to=4-6, "T(f_1)", bend right=25, crossing over, pos=0.2]
\arrow[from=1-7, to=4-6 , "T(f_2)"', bend left=20, crossing over, pos=0.4]            
\end{tikzcd}
        \]
        
        In order to prove that also finite coproducts exists, it suffices to show that the initial object exists. Let $0$ be the initial object in $\mathcal{X}$. It holds that $(T(0),\mu_0)$ is the initial object in $\mathcal{X}^T$. In fact, let $(A,\xi_A)$ be an object in $\mathcal{X}^T$. Since $0$ is initial in $\mathcal{X}$, there exists a unique morphism $!^A:0 \to A$ in $\mathcal{X}$. Then, the morphism $f=\xi_A T(!^A)$ is a morphism in $\mathcal{X}$ from $(T(0),\mu_0)$ to $(A,\xi_A)$.
        \[
        % https://tikzcd.yichuanshen.de/#N4Igdg9gJgpgziAXAbVABwnAlgFyxMJZABgBpiBdUkANwEMAbAVxiRABUAKYgShAF9S6TLnyEUARnJVajFmy4BBPoOHY8BIgCZp1es1aIQigTJhQA5vCKgAZgCcIAWyRSQOCEh2yDbADp+AB5YAPomqiAOzkhk7p6IbvryRlwAhAB6yqb8QA
\begin{tikzcd}
T(0) \arrow[r, "T(!^A)"] & T(A) \arrow[r, "\xi_A"] & A
\end{tikzcd}
        \]
        We aim to prove that it is a morphism in $\mathcal{X}^T$ and that it is the unique morphism in $\mathcal{X}^T$ from $(T(0),\mu_0)$ to $(A,\xi_A)$.
        It is a morphism in $\mathcal{X}^T$, in fact, using property of algebra $\xi_A T(\xi_A) = \xi_A \mu_A$ and the naturality of $\mu$, we have that
        \begin{align*}
            \xi_A T(f) & = \xi_A T(\xi_A T(!^A)) \\[1.5ex]
            & = \xi_A T(\xi_A) T^2(!^A) \\[1.5ex]
            & = \xi_A \mu_A T^2(!^A) \\[1.5ex]
            & = \xi_A T(!^A) \mu_{0} \\[1.5ex]
            & = f \mu_0
        \end{align*}
        \[
        % https://tikzcd.yichuanshen.de/#N4Igdg9gJgpgziAXAbVABwnAlgFyxMJZAFgBoAGAXVJADcBDAGwFcYkQAVAPQCYAKcgEoQAX1LpMufIRRkAjNTpNW7DgOFiJ2PASIA2UgpoMWbRJz4BBDeJAZt0-RUUmV57v2ujb9qbpQ8hi7KZiCW3lp+MsiBVMYhqlY2kTrR5EHxpolemnaSqUTpcUpZ7rxJooowUADm8ESgAGYAThAAtkjpIDgQSADMmW4WAIRcObYt7UhyND1IgSVDaqPjTa0diF1ziDOLoQA6+23MAPrkESCTGwPdvYgLrgdHp+G5V0gGt0hke+yHAB5YE6vCbrJAAVlmdx+jz++0BwIu70QAHYoR9Bk9joi3mDUejEJDfu4+ACgTlKCIgA
\begin{tikzcd}
T^2(A) \arrow[d, "\mu_A"] \arrow[rr, "T(\xi_A)"] &  & T(A) \arrow[d, "\xi_A"] &  & T^2(0) \arrow[rr, "T(!^A)"] \arrow[d, "\mu_0"] &  & T^2(A) \arrow[d, "\mu_A"] \\
T(A) \arrow[rr, "\xi_A"]                         &  & A                       &  & T(0) \arrow[rr, "T(!^A)"]                      &  & T(A)                     
\end{tikzcd}
\]
    Moreover, it is the unique morphism in $\mathcal{X}^T$ from $(T(0),\mu_0)$ to $(A,\xi_A)$. In fact, let $f':(T(0),\mu_0) \to (A,\xi_A)$ be an other morphism in $\mathcal{X}^T$. Then, it holds that
    \begin{itemize}
        \item $f \mu_0 = \xi_A T(f)$
        \item $f' \mu_0 = \xi_A T(f')$
    \end{itemize}
    and, composing both sides with $T(\eta_0)$, we have that
    \begin{itemize}
        \item $f = f 1_{T(0)} = f \mu_0 T(\eta_0) = \xi_A T(f) T(\eta_0) = \xi_A T(f \eta_0) = \xi_A T(!^A)$
        \item $f' = f' 1_{T(0)} = f' \mu_0 T(\eta_0) = \xi_A T(f') T(\eta_0) = \xi_A T(f' \eta_0) = \xi_A T(!^A)$
    \end{itemize}
    since $!^A$ is the unique morphism from $0$ to $A$. Thus $f=f'$.

    It remains to prove that $\mathcal{X}^T$ is finitely cocomplete. We already know that it has finite coproducts, so we only need to show that it has coequalizers. This follows from the fact that it has finite coproducts and reflexive coequalizers; in fact each coequalizer can be constructed as a reflexive coequalizer from the coproduct, as shown below.
        \[
...
\begin{subbox}{}
    \begin{proof}
        Assume $\mathcal{A}$ is exact and Mal'tsev. We want to prove that $\mathcal{A}$ has reflexive coequalizers. Consider a reflexive pair $(f,g)$ from $A$ to $B$. By \Cref{relation induced by a reflexive pair is reflexive} the internal relation induced $\langle r_1,r_2 \rangle$ is reflexive. Since $\mathcal{A}$ is Mal'tsev, then each internal reflexive relation is an equivalence relation. Hence $\langle r_1,r_2 \rangle$ is an equivalence relation. $\mathcal{A}$ is also exact, then, by \Cref{equiv rel is kernel pair of the coeq} the equivalence relation $(r_1,r_2)$ has a coequalizer $k$.
        
        We want to prove that $k$ is also the coequalizer of $(f,g)$. First we note that $kf=kr_1e=kr_2e=kg$. It remains to prove the universal property. Let $h:B \to C$ be a morphism such that $hf=hg$. Then $hr_1e=hr_2e$. Since $e$ is a regular epi, hence by \Cref{reg epi is an epi} an epi, we have that $hr_1=hr_2$. Thus there exists a unique morphism $t$ such that $tk=h$.
        \[
    % https://tikzcd.yichuanshen.de/#-2WICBILB1AhiwBe2sIGyu1aHW6vU2IAGQxGXxAYxOk1Rs3muMQMMuBJCwkRJ3KqIALC0sGBUnAIAwsI8OalnDyaijpoKQBKRWwoHQ5lldnCnrscUsziS1uShJTtjSKFwgA
\begin{tikzcd}
A \arrow[dd, "e", two heads] \arrow[rdd, "f", bend left, shift left] \arrow[rdd, "g"', bend left, shift right] &                                   &                                                  \\
                                                                                                               &                                   & C                                                \\
R \arrow[r, "r_2"', shift right] \arrow[r, "r_1", shift left]                                                  & B \arrow[r, "k"'] \arrow[ru, "h"] & {\mathrm{Coeq}(r_1,r_2)} \arrow[u, "t"', dashed]
\end{tikzcd}
        \]
    \end{proof}    
\end{subbox}
