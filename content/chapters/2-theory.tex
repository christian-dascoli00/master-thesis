...
\begin{rembox}{}
    ...
    The connected limits in the category $(\mathcal{A} \downarrow E)$ can be computed in $\mathcal{A}$. In fact we can provide to the object $L$ of the limit cone $(L,(q_i)_{i \in \mathcal{I}})$ in $\mathcal{A}$ a morphism to form an object in $(\mathcal{A} \downarrow E)$. In the zigzag below we chose for example $(\varphi_1)(q_1)$, but we could use whatever morphism of the type $(\varphi_i)(q_i)$. The connection guarantees that all the morphisms $q_1 \ldots q_n$ involved in the limit in $\mathcal{A}$ are also morphisms in $(\mathcal{A} \downarrow E)$, ie $(\varphi_i)(q_i) = (\varphi_1)(q_1) \ \forall i $.
...
\begin{rembox}{}  
    ...
    \[
% https://tikzcd.yichuanshen.de/#-OzxkoAWCpXrasY6+hAm1vau+yNe-sHN0cKdpH3ltxBj04lp2cWXxBvRhYMAhKAQZgtRhsGi1GD0KDsSCg84ZehYRhIggwkDNNqdbq3XEweJDcKSIA
\begin{tikzcd}
    & A\times_BA' \arrow[ld] \arrow[r] & A' \arrow[ld, "\alpha'"'] \arrow[ldd, "\alpha'"] \\
A \arrow[r, "\alpha"] \arrow[rd, "\alpha"'] & E \arrow[d, no head, Rightarrow] &                                                  \\
    & E                                &                                                 
\end{tikzcd}
\]
...
defined as follows:
\begin{itemize}
\item $p^*: (\mathcal{A} \downarrow B) \to (\mathcal{A} \downarrow E)$ is the pullback functor
\end{itemize}
...
\]
\begin{itemize}
    \item $p_!: (\mathcal{A} \downarrow E) \to (\mathcal{A} \downarrow B)$ is the shrink functor
...
\]
is an adjunction. In particular the unit is given by
...
\[ 
     (\mathrm{proj}_{2,E \times_BA})(\tilde{\mathrm{proj}_2}) (\langle \mathrm{proj}_1, 1_{E\times_BA} \rangle) = 
\]     
\[
 = (\mathrm{proj}_{2,E \times_BA}) (\mathrm{proj}_{2,E\times_B(E\times_BA)}) (\langle \mathrm{proj}_1, 1_{E\times_BA} \rangle) =
\]
\[
...
\begin{mainbox}{}
    ...
    \[
    % https://tikzcd.yichuanshen.de/#N4Igdg9gJgpgziAXAbVABwnAlgFyxMJZAJgBoBmAXVJADcBDAGwFcYkQAdDgW3pwAsATt2ABhCDACOAXwAUaAPoBGUouIBKENNLpMufIRQqqNBizaIQAQS06QGbHgJEADBWp0mrdl14DhwACikrKS6tK2uo4GrqTEHmbelmgAegBUsr58QiLBcmGa2lH6zkZxCV4W9umyVuoAvFZceNzwCgBCgZH2ek6GJOWmleypGVn+IuJScooqauqFdg4l-WQuFebsogDk3ct9setDm5aiWh4wUADm8ESgAGaCENxIKiA4EEhunicgkt2PZ5IMjvT6IN5wfhYe44JAAWjeiSqagBTxeiBBH1eNEh0Nh4OOSXsyhANEY9AARjBGAAFXoxSyCLBXfiwoogQHo8g0LEY9mcpAAFh5YKU-
    \begin{tikzcd}
        C \arrow[rdd, "r1"', shift right] \arrow[rdd, "r_2", shift left] \arrow[dd, "\exists ! s"', dashed]       &                                                                     & C'                                                                       \\
                                                                                                                  &                                                                     &                                                                          \\
        p^*(\mathrm{Eq}(q)) \arrow[d] \arrow[r, "\tilde{p_2}", shift left] \arrow[r, "\tilde{p_1}"', shift right] & p^*(A)=A\times_BE \arrow[d] \arrow[r, "\tilde{q}"] \arrow[ruu, "u"] & {p^*(\mathrm{Coeq}(p_1,p_2))} \arrow[d] \arrow[uu, "\exists! t", dashed] \\
        \mathrm{Eq(q)} \arrow[r, "p_2", shift left] \arrow[r, "p_1"', shift right]                                & A \arrow[r, "q"]                                                    & {\mathrm{Coeq}(p_1,p_2)}                                                
        \end{tikzcd}
    \]
    Let $q$ be the coequalizer of the kernel pair $\mathrm{Eq}(q)$. We can affirm by \Cref{p^*(f) is a pullback} applied to $q$ that the diagram in the bottom-right corner is a pullback. Since by hypothesis $q$ is a pullback stable regular epimorphism, we have that $\tilde{q}$ is a regular epimorphism, ie it is the coequalizer of some parallel arrows. Let $(r_1,r_2)$ be such pair.
    
    Now we want to prove that $\tilde{q}$ is also the coequalizer of $(\tilde{p_1},\tilde{p_2})$. Since $\tilde{q}$, $\tilde{p_1}$ and $\tilde{p_2}$ are obtained applying $p^*$ to the kernel pair $\mathrm{Eq}(q)$, and $p^*$ preserves limits, then $p^*(\mathrm{Eq}(q))$ is the kernel pair of $\tilde{q}$. So by the universal property there exists a unique morphism $s : C \to p^*(\mathrm{Eq}(q))$ such that $\tilde{p_1}s=r_1$ and $\tilde{p_2}s=r_2$. Now let $u$ be a morphism such that $u \tilde{p_1} = u \tilde{p_2}$. We have that $ur_1 = u \tilde{p_1} s = u \tilde{p_2} s = ur_2$, so by the universal property of the coequalizer $\tilde{q}$ there exists a unique morphism $t : p^*(\mathrm{Coeq}(p_1,p_2)) \to C'$ such that $t\tilde{q} = u$. So $\tilde{q}$ is also the coequalizer of $(\tilde{p_1},\tilde{p_2})$.
    \end{proof}
    \end{subbox}
...
\begin{mainbox}{}
\begin{propr}
Let $p : E \to B$ be a morphism in $\mathcal{A}$ with pullbacks. The monad $(p^*p_!, \eta, p^*\epsilon p_!)$ is cartesian.
\end{propr}
\end{mainbox}
\begin{subbox}{}
\begin{proof}
...
\begin{lemma}
    ...
    \[
% https://tikzcd.yichuanshen.de/#
\begin{tikzcd}
    B'\times_BA \arrow[rr, "u"] \arrow[d, "q"', shift right] &  & A \arrow[d, "p"', shift right] \\
    B' \arrow[rr, "v"] \arrow[u, "t"', shift right]          &  & B \arrow[u, "s"', shift right]
    \end{tikzcd}
    \]
...
\begin{subbox}{}
\begin{proof}
Assume $\mathcal{X}$ is protomodular. We want to prove that if $u$ and $s$ factorize through a mono $m$ as $u = mf$ and $s = ms'$, then $m$ is an iso. By (cita appendice), this implies that $(u,s)$ is a strongly epimorphic pair.
Define $p' = pm$, we have that $p's'=1_Y$, in fact $p's'=pms'=ps=1_Y$. Pulling back with the change-of-base functor $v^*$ the mono $m$ along $v$ we obtain the following diagram. In this operation we also generate the morphism $q'$, $t'$, $w$. In the same way as in \Cref{p^*(f) is a pullback} we have that the diagram between $m$ and $v^*(m)$ is a pullback.
\[
