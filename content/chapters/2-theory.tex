...
\begin{subbox}{}
    \begin{proof}
        ...
        First we show that $\mathcal{X}^T$ is finitely complete. We recall that the forgetful functor $U^T:\mathcal{X}^T \to \mathcal{X}$ creates all the limits that $\mathcal{X}$ has. Since $\mathcal{X}$ is exact, it has all finite limits. Therefore, $\mathcal{X}^T$ has all finite limits. (TO VERIFY)
        ...
        \[
        % https://tikzcd.yichuanshen.de/#-SmPnYf0Bs1kcOWiCRfSQAFZvr47GknNVavVdE0WlT2vIXkTkcUSeiAUCDKDwb4oPQhglFvDEO8CXiNsNRhNUXs-LY5uRAozhaLPvjNlKdsczhdrsZQiAMvdHkdptF5bNVHFuEA
\begin{tikzcd}
    T(R) \arrow[d, "\xi_R"] \arrow[r, "T(r_1)", shift left] \arrow[r, "T(r_2)"', shift right] & T(A) \arrow[d, "\xi_A"] \arrow[r, "T(f)", two heads] & T(B) \arrow[d, "\xi_B", dashed] \\
    R \arrow[r, "r_1", shift left] \arrow[r, "r_2"', shift right]                             & A \arrow[r, "f", two heads]                          & B                              
    \end{tikzcd}
    \]

        (TO ADJUST)
        Consider now a morphism $f:(A,\xi_A)\to (B, \xi_B)$ in $\mathcal{X}^T$, we aim to prove that it admits a (regular epi, mono) factorization. The morphism $f$, under the forgetful functor $U^T$, is a morphism in $\mathcal{X}$, which is exact and in particular regular. Thus $f$ admits a (regular epi, mono) factorization in $\mathcal{X}$, let it be $f=mq$ with $m:Q \to B$ mono and $q: A \to Q$ regular epi. Notice that $A$ and $B$ are already algebras, we want to provide $Q$ with the structure of an algebra, so that we can lift the factorization to $\mathcal{X}^T$.

        In order to do so, we first need to show that $T$ preserves coequalizers in $\mathcal{X}$ of kernel pairs arising from $\mathcal{X}^T$, once the algebra structure is forgotten. Let $(\mathrm{Eq}(p),r_1,r_2)$ be the kernel pair in $\mathcal{X}$ of a morphism $p$, arising from a kernel pair in $\mathcal{X}^T$, and suppose that $p$ is coequalizer in $\mathcal{X}$. Given that $(\mathrm{Eq}(p),r_1,r_2)$ is a kernel pair arising from $\mathcal{X}^T$, it follows that $(T(\mathrm{Eq}(p)),T(r_1),T(r_2))$ is the kernel pair of $T(p)$ in $\mathcal{X}$. Since $T$ preserves regular epi, we have that $T(p)$ is a regular epi. By \Cref{coeq of the kernel pair}, we can assert that $T(p)$ is the coequalizer of the kernel pair $(T(\mathrm{Eq}(p)),T(r_1),T(r_2))$, as claimed.
        
        We know that $U^T$ creates all the colimits that $\mathcal{X}$ has and $T$ preserves. Hence, $U^T$ creates all coequalizers that $\mathcal{X}$ has of kernel pairs arising from $\mathcal{X}^T$, once the algebra structure is forgotten. 

        Picking up where we left off, notice that $q$ is a regular epi. By \Cref{coeq of the kernel pair} we can assume that $q$ is the coequalizer of its kernel pair $(\mathrm{Eq}(q),r_1,r_2)$. Since $U^T$ creates all the limits that $\mathcal{X}$ has, we can lift the kernel pair $(\mathrm{Eq}(q),r_1,r_2)$ to a kernel pair in $\mathcal{X}^T$. Let $\xi$ be the algebra of $\mathrm{Eq}(q)$. We observe that $q$ is a coequalizer of the kind preserved by the monad $T$. Hence, $q$ is also a coequalizer in $\mathcal{X}^T$. This means that $q$ is a regular epi in $\mathcal{X}^T$ and $Q$ has an algebra structure $\xi_Q$.
        
        (VERIFY UNTIL HERE)

        In order to prove the factorization of $f$ in $\mathcal{X}^T$, it remains to establish that $m$ is a morphism in $\mathcal{X}^T$ and that it is a mono. Notice that $m$ is a mono in $\mathcal{X^T}$, in fact it is a mono in $\mathcal{X}$ and furthermore it is merely a morphism $\mathcal{X}$ that satisfies the algebra condition. Finally, we want to establish that $m$ is a morphism in $\mathcal{X}^T$.
