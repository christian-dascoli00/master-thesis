...
\begin{tikzcd}
    ...
    \end{tikzcd}
        ...
        Picking up where we left off, consider $f$ and its kernel pair $(\mathrm{Eq}(q),r_1,r_2)$. Since $U^T$ creates all the limits that $\mathcal{X}$ has, we can lift the kernel pair $(\mathrm{Eq}(q),r_1,r_2)$ to a kernel pair in $\mathcal{X}^T$. Let $\xi_\mathrm{Eq}$ be the algebra of $\mathrm{Eq}$.
