...
\begin{subbox}{}
    \begin{proof}
        ...
        In order to do that we need first to prove that $T$ preserves coequalizers of kernel pairs. Let $p: X \to Y$ be a coequalizer and let $(\mathrm{Eq}(p),r_1,r_2)$ be its kernel pair. $T$ preserves kernel pair and regular epi by hypothesis, thus $T(p)$ is a regular epi and $(T(\mathrm{Eq}(p)),T(r_1),T(r_2)$ is the kernel pair of $T(p)$. By \Cref{coeq of the kernel pair} we can assert that $T(p)$ is the coequalizer of the kernel pair $(T(\mathrm{Eq}(p)),T(r_1),T(r_2)$, as claimed. We know that $U^T$ creates all the colimits that $\mathcal{X}$ has and $T$ preserves. Since $T$ preserves coequalizers of kernel pairs, we can conclude that $U^T$ creates all coequalizers of kernel pairs that $\mathcal{X}^T$ has.
        
        Picking up where we left off, notice that $q$ is a regular epi. By \Cref{coeq of the kernel pair} we can assume that $q$ is the coequalizer of its kernel pair $(\mathrm{Eq}(q),r_1,r_2)$. 
        

        Da qui da rivedere.!!!

        
        Since $U^T$ creates all the limits that $\mathcal{X}$ has, we have that $(\mathrm{Eq}(q),r_1,r_2)$ can be lifted to a kernel pair in $\mathcal{X}^T$. Let $\xi$ be the structure of an algebra on $\mathrm{Eq}(q)$. Moreover, since $T$ preserves regular epi, we have that $T(q)$ is a regular epi. By \Cref{coeq of the kernel pair}, $T(q)$ is the coequalizer of its kernel pair $(T(\mathrm{Eq}(q)),T(r_1),T(r_2))$. 
        ...
        We have that $q \xi_A T(r_1) = q r_1 \xi = q r_2 \xi = q \xi_A T(r_2)$, thus, by the universal property of the coequalizer $T(q)$, there exists a unique morphism $\xi_Q: T(Q) \to Q$ such that $\xi_Q T(q) = Q \xi_A$. This morphism is the structure of an algebra on $Q$. 
