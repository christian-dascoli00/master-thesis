...
% bibliography
\usepackage[hyperref=true,backend=biber,maxbibnames=9,maxcitenames=2,style=numeric-comp,sorting=nty,backref=false]{biblatex} % hyperref uses links, backref goes back to citations, uses biber as backend, with 9 names at most in bibliography and 2 in citations, citing using numbers, and sorting in citation order
...
\usepackage{amssymb}   % per \twoheadrightarrow
\usepackage{mathtools} % per \rightarrowtail
