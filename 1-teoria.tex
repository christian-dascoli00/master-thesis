...
Let us show that this is actually an adjunction. First we prove that the unit is a natural transformation. Let $g : (D,\delta)  \to ((D',\delta'))$ be a morphism in $(\mathcal{A} \downarrow E)$, we want to prove that the following diagram commutes:
...
\noindent we can prove the naturality in the left diagram by the universal property of the pullback in the right. Since $(\mathrm{proj}_1$, $\mathrm{proj}_2)$ are jointly monic, by the following compositions
...
\noindent we conclude that ($\tilde{g}) (\eta_{(D,\delta)}) = (\eta_{(D',\delta')}) (g)$. So the naturality condition of $\eta$ is satisfied.

Now we want to prove the counit is a natural trasfromation. Consider $f : A \to A'$ a morphism in $(\mathcal{A} \downarrow B)$, we want to prove that the following diagram commutes:

\[
% https://tikzcd.yichuanshen.de/#-ZKReUilRanbmXsHWEdgJ2f6F04Vxu+juAXEQA
\begin{tikzcd}
...
Considering the of the functor $p_!p^*$ on the morphism $f$, we have:
...
\[
% https://tikzcd.yichuanshen.de/#-QHccGCeGrFGJJQJEA
\begin{tikzcd}
...
The naturaity follow (to complete)

We can check that the unit and counit fulfil the first triangle identities. Let $(A,\alpha)$ be an object in $(\mathcal{A} \downarrow B)$, we have:
...
\begin{tikzcd}
    E\times_BA \arrow[rd, "{\alpha (\mathrm{proj}_{2,E\times_BA})}"'] \arrow[rr, "{\mathrm{proj}_{2,E\times_BA}}"] &   & A \arrow[ld, "\alpha"] \\
                                                                                                 & B &                       
    \end{tikzcd}
\]
...
\noindent and the second composition
