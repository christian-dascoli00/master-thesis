...
\begin{rem}
$p^*$ is a left adjoint functor, so it preserves limits. $p_!$ is a right adjoint functor, so it preserves colimits.
\end{rem}
...
In order to prove that $\eta$ is cartesian, ie the naturality diagrams are pullback, we consider the bottom and the total comuttative diagrams below. Let $g : (D,\delta) \to (D',\delta')$ be a morphism in (A E). Since both the diagrams are pullback, it follows that the top diagram is a pullback.
\[
...
Considering the following commutative diagram, since the right diagram is a pullabck, as we just proved, and the total diagram is a pullback, we can conclude that the left diagram is a pullback, as stated.
\[
