...
These notes are based on the work of Sobral and Tholen \cite{SobralTholen1992}. This version includes revised proofs adapted to the specific needs of our case. Some ideas are drawn from Janelidze and Tholen \cite{JanelidzeTholen1991}. Our goal is to prove the following theorem.
...
We recall that the unit $\eta$ of the monad in an object $(C,\gamma)$ in $(\mathcal{A} \downarrow E)$ is $\langle \gamma, 1_C \rangle$ and the counit $\epsilon$ in an object $(D,\zeta)$ in $(\mathcal{A} \downarrow B)$ is the morphism $\mathrm{proj}_{2,D}: E\times_BD \to D$ given by the pullback of $p$ along $\zeta: D \to B$.
