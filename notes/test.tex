...
\usepackage[
  top=2.5cm,    % margine superiore stretto
  bottom=2.5cm, % margine inferiore stretto
  left=3cm,     % margine sinistro (modifica a piacere)
  right=3cm     % margine destro (modifica a piacere)
]{geometry}
\usepackage{lmodern}
...
Hence (Does $\xi$ coeq $(1_E\times_B\mathrm{proj}_{2,C}, 1_E\times_B\xi)$ ??)
...
\begin{proof}
    The exactness of the category implies that the comparison functor $K:(\mathcal{A} \downarrow B) \to (\mathcal{A} \downarrow E)^T$ has a left adjoint $L:(\mathcal{A} \downarrow E)^T \to (\mathcal{A} \downarrow B)$, as shown. Since $p$ is a regular epi, the unit $\alpha$ and the counit $\rho$ of the adjunction $L \dashv K$ are pointwise isomorphisms, by \ref{counit iso} and \ref{unit iso}. Thus, $L \dashv K$ is an adjoint equivalence. This implies that $K$ is an equivalence of categories. So $p^*: (\mathcal{A} \downarrow B) \to (\mathcal{A} \downarrow E)$ is a monadic functor.
\end{proof}
