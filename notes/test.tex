...
\title{Regular epis and monadic $p^*$ in an exact category}
...
Consider the comparison functor $K:(\mathcal{A} \downarrow B) \to (\mathcal{A} \downarrow E)^T$ defined as $K(D,\zeta)=(p^*(D,\zeta),p^*(\epsilon_{(D,\zeta}))$, where $\epsilon$ is the counit of the adjunction, definig $\epsilon_{(D,\zeta)}$ to be the projection from $E\times_BD$ to $D$ of the pullback of $p$ along $\zeta$.
...
\begin{remark}
    $\rho$ pointwise isomorphism $\iff$ $K$ fully faithful $\iff$ $\epsilon$ pointwise regular epi
\end{remark}    
